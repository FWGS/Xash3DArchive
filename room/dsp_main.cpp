...
dly_t * DLY_AllocLP( int D, int a, int b, int type, int M, int L, int *fa, int *fb )
{
	...
	if( i == CDLYS )
	{
		Con_DPrintf( "DSP: failed to allocate delay line.\n" );
	...
	if( type == DLY_LOWPASS )
	{
		...
		if( !pdly->pflt )
		{
			Con_DPrintf( "DSP: failed to allocate filter for delay line.\n" );
			return NULL;
		}
	}
	...
	if( !w )
	{ 
		Con_DPrintf( "DSP: Failed to lock.\n" );
		GlobalFree( h );
		FLT_Free( pdly->pflt );
		return NULL; 
	}
	...
	case DLY_LOWPASS: 
		{
			...
			if( !pflt )
			{
				Con_DPrintf( "DSP: failed to allocate filter.\n" );
				return NULL;
			}
...
rva_t *RVA_Alloc( int *D, int *a, int *b, int m, flt_t *pflt, int fparallel )
{
	...
	if( i == CRVAS )
	{
		Con_DPrintf( "DSP: failed to allocate reverb.\n" );
		return NULL;
	}
	...
	if( pflt && !fparallel )
	{
		...
		if( !pflt2 )
		{
			Con_DPrintf( "DSP: failed to allocate flt for reverb.\n" );
			return NULL;
		}
	}
	...
	for( i = 0; i < m; i++ )
	{
		if( !prva->pdlys[i] )
		{
			FLT_Free( pflt2 );
			RVA_Free( prva );
			Con_DPrintf( "DSP: failed to allocate delay for reverb.\n" );
			return NULL;
		}
	}
...
dfr_t *DFR_Alloc( int *D, int *a, int *b, int n )
{
	...
	if( i == CDFRS )
	{
		Con_DPrintf( "DSP: failed to allocate diffusor.\n" );
		return NULL;
	}
	...
	for( i = 0; i < n; i++ )
	{
		...
		{
			DFR_Free( pdfr );
			Con_DPrintf( "DSP: failed to allocate delay for diffusor.\n" );
			return NULL;
		}
	}
...
lfo_t *LFO_Alloc( int wtype, float freqHz, qboolean foneshot )
{
	...
	Con_DPrintf( "DSP: failed to allocate LFO.\n" );
	return NULL;
}
...
ptc_t * PTC_Alloc( float timeslice, float timexfade, float fstep ) 
{
	...
	if( i == CPTCS ) 
	{
		Con_DPrintf( "DSP: failed to allocate pitch shifter.\n" );
		return NULL;
	}
	...
	{
		PTC_Free( pptc );
		Con_DPrintf( "DSP: failed to allocate delay for pitch shifter.\n" );
		return NULL;
	}
...
env_t *ENV_Alloc( int type, float famp1, float famp2, float famp3, float attack, float decay, float sustain, float release )
{
	...
	Con_DPrintf( "DSP: failed to allocate envelope.\n" );
	return NULL;
}
...
efo_t *EFO_Alloc( void )
{
	...
	Con_DPrintf( "DSP: failed to allocate envelope follower.\n" );
	return NULL;
}
...
mdy_t *MDY_Alloc( dly_t *pdly, float ramptime, float modtime, float depth )
{
	...
	for( i = 0; i < CMDYS; i++ )
	{
		if( !mdys[i].fused )
		{
			...
			if( !pmdy->pdly )
			{
				Con_DPrintf( "DSP: failed to allocate delay for mod delay.\n" );
				return NULL;
			}
	...
	Con_DPrintf( "DSP: failed to allocate mod delay.\n" );
	return NULL;
}
...
crs_t *CRS_Alloc( int lfotype, float fHz, float fdepth, float mix ) 
{
	...
	if( i == CCRSS ) 
	{
		Con_DPrintf( "DSP: failed to allocate chorus.\n" );
		return NULL;
	}
	...
	if( !plfo || !pmdy )
	{
		LFO_Free( plfo );
		MDY_Free( pmdy );
		Con_DPrintf( "DSP: failed to allocate lfo or mdy for chorus.\n" );
		return NULL;
	}
...
amp_t *AMP_Alloc( float gain, float vthresh, float distmix, float vfeed ) 
{
	...
	if( i == CAMPS ) 
	{
		Con_DPrintf( "DSP: failed to allocate amp.\n" );
		return NULL;
	}
...
void PRC_CheckParams( prc_t *pprc, prm_rng_t *prng )
{
	...
	for( i = 0; i < cprm; i++)
	{
		...
		{
			Con_DPrintf( "DSP: clamping out of range parameter.\n" );
...
pset_t *PSET_Alloc( int ipsettemplate )
{
	...
	*ppset = psettemplates[ipsettemplate];
	...
	if( !fok )
	{
		...
		Con_DPrintf( "DSP: preset failed to init.\n");
		PRC_FreeAll( ppset->prcs, ppset->cprcs );
		return NULL;
	}
	return ppset;
...
#define DSPCHANMAX		2	// max number of channels dsp can process (allocs a separte processor for each chan)
...
void DSP_SetPreset( int idsp, int ipsetnew )
{
	...
	for( i = 0; i < pdsp->cchan; i++ )
	{
		...
		if( !ppsetnew[i] )
		{
			Con_DPrintf( "DSP preset failed to allocate.\n" );
			return;
		}
	}
...
void DSP_Process( int idsp, void *channels, int sampleCount )
{
	...
	pdsp = &dsps[idsp];
	pbfront = (portable_samplepair_t *)channels;
