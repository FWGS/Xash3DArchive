...
void EmitFace (face_t *f)
{
	...
	if (numfaces >= MAX_MAP_FACES)
		Sys_Error ("numfaces == MAX_MAP_FACES");
	df = &dfaces[numfaces];
	numfaces++;
	...
	for (i=1 ; i<num_entities ; i++)
	{
		...
		if (j == stylenum)
		{
			if (stylenum == MAX_SWITCHED_LIGHTS)
				Sys_Error ("stylenum == MAX_SWITCHED_LIGHTS");
