...
void ParsePatch( void )
{
	...
	Com_ReadToken( mapfile, SC_ALLOW_NEWLINES|SC_ALLOW_PATHNAMES, &token );
	com.strncpy( texture, token.string, MAX_STRING );
	...
	for( j = 0; j < m.width ; j++ )
	{
		...
		{
			Com_Parse1DMatrix( mapfile, 5, verts[i*m.width+j].point );
	...
	if( g_brushtype == BRUSH_RADIANT && com.strcmp( token.string, "}" ))
	{
		ep = ParseEpair( &token );
	}
