...
void CM_TracePointThroughSurfaceCollide(traceWork_t * tw, const cSurfaceCollide_t * sc)
{
	...
	for(i = 0, facet = sc->facets; i < sc->numFacets; i++, facet++)
	{
		...
		if(j == facet->numBorders)
		{
			...
			if(!cv)
			{
				cv = Cvar_Get( "r_debugSurfaceUpdate", "1", 0, "no description" );
			}
...
void CM_TraceThroughSurface(traceWork_t * tw, csurface_t * surface)
{
	...
	if( cm_triangles->integer && surface->type == MST_TRISURF && surface->sc)
	{
		CM_TraceThroughSurfaceCollide(tw, surface->sc);
	}
...
static void CM_ProximityToBrush(traceWork_t * tw, cbrush_t * brush)
{
	...
	Mem_Set(&tw2, 0, sizeof(tw2));

	tw2.trace.flFraction = 1.0f;
	tw2.type = TR_CAPSULE;
	...
	if(tw2.trace.flFraction == 1.0f && !tw2.trace.fAllSolid && !tw2.trace.fStartSolid)
	{
		...
		if(fraction < tw->trace.flRealFrac)
			tw->trace.flRealFrac = fraction;
	}
	else
		tw->trace.flRealFrac = 0.0f;
}
...
void CM_TraceThroughLeaf(traceWork_t * tw, cleaf_t * leaf)
{
	...
	if(tw->lateralCollision && tw->trace.flFraction < 1.0f)
	{
		for(k = 0; k < leaf->numleafbrushes; k++)
		{
			brushnum = cm.leafbrushes[leaf->firstleafbrush + k];
...
==================
*/
static void CM_Trace(TraceResult * results, const vec3_t start,
			  const vec3_t end, vec3_t mins, vec3_t maxs,
			  model_t model, const vec3_t origin, int brushmask, trType_t type, sphere_t * sphere)
{
	int             i;
	traceWork_t     tw;
	vec3_t          offset;
	...
	tw.trace.flFraction = 1;		// assume it goes the entire distance until shown otherwise
	...
	{
		VectorLerp(start, tw.trace.flFraction, end, tw.trace.vecEndPos);
	}
...
//	assert(!tw.trace.fAllSolid);
//	assert(tw.trace.flFraction != 1.0);
//	assert(VectorLength(tw.trace.vecPlaneNormal) > 0.9999);
...
==================
*/
void CM_BoxTrace(TraceResult * results, const vec3_t start, const vec3_t end,
				 vec3_t mins, vec3_t maxs, model_t model, int brushmask, trType_t type)
{
...
==================
*/
void CM_BiSphereTrace(TraceResult * results, const vec3_t start,
					  const vec3_t end, float startRad, float endRad, model_t model, int mask)
{
	int             i;
	traceWork_t     tw;
	...
	tw.type = TR_BISPHERE;
	tw.lateralCollision = true;
	tw.trace.flRealFrac = 1.0f;
	...
	{
		for(i = 0; i < 3; i++)
			tw.trace.vecEndPos[i] = start[i] + tw.trace.flFraction * (end[i] - start[i]);
	}
...
==================
*/
void CM_TransformedBiSphereTrace(TraceResult * results, const vec3_t start,
								 const vec3_t end, float startRad, float endRad,
								 model_t model, int mask, const vec3_t origin)
{
	TraceResult         trace;
	vec3_t          start_l, end_l;
