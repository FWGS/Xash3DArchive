...
//		         vmatrix.cpp - shared vmatrix operations 
//=======================================================================
...
void VMatrix::SetupMatrixOrgAngles( const Vector &origin, const Vector &vAngles )
{
...
void MatrixBuildRotation( VMatrix &dst, const Vector& initialDirection, const Vector& finalDirection )
{
	...
	else if (angle + 1.0 < 1e-3)
	{
		...
		axis.Init( 0, 0, 0 );
		axis[idx] = 1.0f;
		axis.MA( -axis.Dot( finalDirection ), finalDirection, axis );
		axis.Normalize();
		angle = 180.0f;
	}
	else
	{
		axis = finalDirection.Cross( finalDirection );
		axis.Normalize();
