...
void CHalfLifeMultiplay :: Think ( void )
{
	...
	{
		CVAR_SET_STRING( "mp_fragsleft", UTIL_VarArgs( "%i", frags_remaining ) );
	}

	// Updates once per second
	if ( timeleft->value != last_time )
	{
		CVAR_SET_STRING( "mp_timeleft", UTIL_VarArgs( "%i", time_remaining ) );
	}
...
void CHalfLifeMultiplay :: InitHUD( CBasePlayer *pl )
{
	...
	if ( g_fGameOver )
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgIntermission, NULL, pl->edict() );
		MESSAGE_END();
	}
}
...
{
	int iFallDamage = falldamage->integer;
...
{
	return ( allowmonsters->integer != 0 );
}
...
void CHalfLifeMultiplay :: GoToIntermission( void )
{
	...
	MESSAGE_BEGIN(MSG_ALL, gmsgIntermission);
	MESSAGE_END();
	...
	m_flIntermissionEndTime = gpGlobals->time + ( mp_chattime->integer );
