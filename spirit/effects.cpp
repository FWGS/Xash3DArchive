...
{
public: 
	void PostSpawn( void ) { SetObjectClass( ED_SKYPORTAL ); }
};
...
void CParticle::Spawn( void )
{
	...
	pev->renderfx = (pev->spawnflags & SF_PARTICLE_ON) ? kRenderFxAurora : kRenderFxNone;
...
{
	// NOTE: precacheFunc can change particle name
	// to default error particle script
	pev->message = PRECACHE_PARTICLE( pev->message );
}
...
{
	if ( pev->spawnflags & SF_PARTICLE_SPAWNUSE )
	{
		...
		if ( pev->netname != NULL )
		{	
			// set childrens name (targetname) from netname
			pParticle->pev->targetname = pev->netname;	
		}
		...
		pParticle->Spawn();
		pParticle->pev->renderfx = kRenderFxAurora; // turn children on automatically
		pParticle->Think();
	}
	else
	{
		if ( ShouldToggle( useType, pev->renderfx ) )
		{
			if( pev->renderfx == kRenderFxAurora )
				pev->renderfx = kRenderFxNone;
			else pev->renderfx = kRenderFxAurora;
		}
	}
}
...
//=========================================================
void CRainSettings::Precache( void )
{
	if( Rain_Mode )
	{
		// snow
	...
	{
		// it's a rainy day :)
		PRECACHE_MODEL( "sprites/hi_rain.spr" );
		PRECACHE_MODEL( "sprites/waterring.spr" );
	}
}
