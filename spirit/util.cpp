...
int PRECACHE_PARTICLE( string_t s, char *e )
{
	if (FStringNull( s )) 
		return PRECACHE_PARTICLE( e );
	return PRECACHE_PARTICLE( s );
}
int PRECACHE_PARTICLE( string_t s ){ return PRECACHE_PARTICLE( (char*)STRING(s)); }
int PRECACHE_PARTICLE( char* s )
{
	if ( !s || !*s )
	{
		ALERT( at_console, "Warning: particlename not specified\n" );
		return MAKE_STRING( "aurora/error.aur" ); // assume error
	}
...
void UTIL_HudMessage( CBaseEntity *pEntity, const hudtextparms_t &textparms, const char *pMessage )
{
	...
	MESSAGE_BEGIN( MSG_ONE, gmsgTempEntity, NULL, pEntity->edict() );
...
{
	// NOTE: actual mins\maxs will be set automatically 
