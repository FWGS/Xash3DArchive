...
static int FindMetaVertex( bspDrawVert_t *src )
{
	...
	if( numMetaVerts >= maxMetaVerts )
	{
		...
		temp = Malloc( maxMetaVerts * sizeof( bspDrawVert_t ) );
		if( metaVerts != NULL )
		{
...
static int AddMetaTriangle( void )
{
	...
	if( numMetaTriangles >= maxMetaTriangles )
	{
		...
		temp = Malloc( maxMetaTriangles * sizeof( metaTriangle_t ) );
...
int FindMetaTriangle( metaTriangle_t *src, bspDrawVert_t *a, bspDrawVert_t *b, bspDrawVert_t *c, int planeNum )
{
	...
	else
	{
		...
		src->planeNum = -1;
		if( PlaneFromPoints( src->plane, a->xyz, b->xyz, c->xyz ) == false )
			return -1;
	}
...
void TriangulatePatchSurface( mapDrawSurface_t *ds )
{
	...
	ds->type = SURFACE_META;
	ds->numIndexes = 0;
	ds->indexes = Malloc( mesh->width * mesh->height * 6 * sizeof( int ) );
...
void FanFaceSurface( mapDrawSurface_t *ds )
{
	...
	verts = Malloc( (ds->numVerts + 1) * sizeof( bspDrawVert_t ) );
	...
	ds->numIndexes = 0;
	ds->indexes = Malloc( ds->numVerts * 3 * sizeof( int ) );
...
void StripFaceSurface( mapDrawSurface_t *ds ) 
{
	...
	else
	{
		...
		least = 0;
		if( ds->shaderInfo != NULL && ds->shaderInfo->autosprite == false )
		{
		...
		if( numIndexes > MAX_INDEXES )
			Sys_Error( "MAX_INDEXES exceeded for surface (%d > %d) (%d verts)", numIndexes, MAX_INDEXES, ds->numVerts );
...
void MakeEntityMetaTriangles( entity_t *e )
{
	...
	for( i = e->firstDrawSurf; i < numMapDrawSurfs; i++ )
	{
		...
		/* meta this surface? */
		if( meta == false && ds->shaderInfo->forceMeta == false )
			continue;
...
{
	int		i, j, k, f, fOld, vertIndex, triIndex, numTJuncs;
	...
	/* note it */
	MsgDev( D_INFO, "--- FixMetaTJunctions ---\n" );
	...
	/* emit some stats */
	MsgDev( D_INFO, "%9d T-junctions added\n", numTJuncs );
}
...
{
	int			i, j, k, f, fOld, cs, numVerts, numVotes, numSmoothed;
	...
	for( i = 0; i < numMetaVerts; i++ )
	{
		...
		for( j = i; j < numMetaVerts && numVerts < MAX_SAMPLES; j++ )
		{
			...
			/* test vertexes */
			if( VectorCompare( metaVerts[ i ].xyz, metaVerts[ j ].xyz ) == false )
				continue;
...
int AddMetaVertToSurface( mapDrawSurface_t *ds, bspDrawVert_t *dv1, int *coincident )
{
	...
	for( i = 0; i < ds->numVerts; i++ )
	{
		...
		if( VectorCompare( dv1->xyz, dv2->xyz ) == false )
			continue;
...
static int AddMetaTriangleToSurface( mapDrawSurface_t *ds, metaTriangle_t *tri, bool testAdd )
{
	...
	if( ds->shaderInfo != tri->si || ds->fogNum != tri->fogNum || ds->sampleSize != tri->sampleSize )
		...
		if( !(ds->shaderInfo->compileFlags & C_VERTEXLIT) &&
			//% VectorCompare( ds->lightmapAxis, tri->lightmapAxis ) == false )
			DotProduct( ds->lightmapAxis, tri->plane ) < 0.25f )
			return 0;
	#endif
	
	/* planar surfaces will only merge with triangles in the same plane */
	if( npDegrees == 0.0f && ds->shaderInfo->nonplanar == false && ds->planeNum >= 0 )
	{
...
static void MetaTrianglesToSurface( int numPossibles, metaTriangle_t *possibles, int *fOld, int *numAdded )
{
	...
	/* allocate arrays */
	verts = Malloc( sizeof( *verts ) * maxSurfaceVerts );
	indexes = Malloc( sizeof( *indexes ) * maxSurfaceIndexes );
	...
	for( i = 0, seed = possibles; i < numPossibles; i++, seed++ )
	{
		...
		ds->verts = Malloc( ds->numVerts * sizeof( bspDrawVert_t ) );
...
static int CompareMetaTriangles( const void *a, const void *b )
{
	...
	/* then plane */
	#if 0
		else if( npDegrees == 0.0f && ((metaTriangle_t*) a)->si->nonplanar == false &&
...
void MergeMetaTriangles( void )
{
	...
	/* emit some stats */
	MsgDev( D_INFO, "%9d surfaces merged\n", numMergedSurfaces );
	MsgDev( D_INFO, "%9d vertexes merged\n", numMergedVerts );
}
