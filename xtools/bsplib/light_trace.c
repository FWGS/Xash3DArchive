...
static int AddTraceInfo( traceInfo_t *ti )
{
	...
	if( numTraceInfos >= maxTraceInfos )
	{
		...
		temp = Malloc( maxTraceInfos * sizeof( *traceInfos ) );
		if( traceInfos != NULL )
		{
...
static int AllocTraceNode( void )
{
	...
	if( numTraceNodes >= maxTraceNodes )
	{
		...
		temp = Malloc( maxTraceNodes * sizeof( traceNode_t ) );
		if( traceNodes != NULL )
		{
...
static int AddTraceWinding( traceWinding_t *tw )
{
	...
	else
	{
		...
		if( numTraceWindings >= maxTraceWindings )
		{
			...
			temp = Malloc( maxTraceWindings * sizeof( *traceWindings ) );
...
static int AddTraceTriangle( traceTriangle_t *tt )
{
	...
	else
	{
		...
		if( numTraceTriangles >= maxTraceTriangles )
		{
			...
			temp = Malloc( maxTraceTriangles * sizeof( *traceTriangles ) );
...
static int AddItemToTraceNode( traceNode_t *node, int num )
{
	...
	if( node->numItems >= node->maxItems )
	{
		...
		if( node->maxItems <= 0 )
			node->maxItems = GROW_NODE_ITEMS;
		temp = Malloc( node->maxItems * sizeof( *node->items ) );
		if( node->items != NULL )
		{
		...
		for( i = 0; i < tw->numVerts; i++ )
		{
			...
			case SIDE_FRONT:
				if( front->numVerts >= MAX_TW_VERTS )
					Sys_Error( "MAX_TW_VERTS (%d) exceeded", MAX_TW_VERTS );
...
static void FilterTraceWindingIntoNodes_r( traceWinding_t *tw, int nodeNum )
{
	...
	{	
		...
		if( node->children[ 0 ] == 0 || node->children[ 1 ] == 0 )
			Sys_Error( "Invalid tracenode: %d", nodeNum );
...
static void SubdivideTraceNode_r( int nodeNum, int depth )
{
	...
	frontNode->items = Malloc( frontNode->maxItems * sizeof( *frontNode->items ) );
	
	/* setup back node */
	backNode->maxItems = (node->maxItems >> 1);
	backNode->items = Malloc( backNode->maxItems * sizeof( *backNode->items ) );
...
static int TriangulateTraceNode_r( int nodeNum )
{
	...
	node->items = Malloc( node->maxItems * sizeof( tt ) );
...
static void PopulateWithBSPModel( bspModel_t *model, matrix4x4 transform )
{
		...
		case MST_PATCH:
			...
			for( y = 0; y < (mesh->height - 1); y++ )
			{
				for( x = 0; x < (mesh->width - 1); x++ )
				{
					...
					Matrix4x4_TransformPoint( transform, tw.v[0].xyz );
					Matrix4x4_TransformPoint( transform, tw.v[1].xyz );
					Matrix4x4_TransformPoint( transform, tw.v[2].xyz );
...
static void PopulateWithPicoModel( int castShadows, picoModel_t *model, matrix4x4 transform )
{
	...
	for( i = 0; i < numSurfaces; i++ )
	{
		...
		for( j = 0; j < numIndexes; j += 3, indexes += 3 )
		{
			for( k = 0; k < 3; k++ )
			{
				...
				Matrix4x4_TransformPoint( transform, tw.v[k].xyz );
			}
...
void SetupTraceNodes( void )
{
...
#if 0
	{
		...
		strcat( filename, ".lin" );
		Msg( "Opening light trace file %s...\n", filename );
		file = FS_Open( filename, "w" );
		if( file == NULL )
			Sys_Error( "Error opening %s for writing", filename );
...
bool TraceTriangle( traceInfo_t *ti, traceTriangle_t *tt, light_trace_t *trace )
{
	int		i;
	...
	else if( trace->recvShadows > 1 )
	{
		if( ti->castShadows != 1 && abs( ti->castShadows ) != abs( trace->recvShadows ) )
			return false;
		//	Msg( "%d:%d ", tt->castShadows, trace->recvShadows );
	}
	...
	if( !(si->compileFlags & (C_ALPHASHADOW | C_LIGHTFILTER)) || si->lightImage == NULL || si->lightImage->pic == NULL )
	{
	...
	s = s - floor( s );
	t = t - floor( t );
	is = s * si->lightImage->pic->width;
	it = t * si->lightImage->pic->height;
	
	/* get pixel */
	pixel = si->lightImage->pic->buffer + 4 * (it * si->lightImage->pic->width + is);
...
bool TraceWinding( traceWinding_t *tw, light_trace_t *trace )
{
	int		i;
	traceTriangle_t	tt;
...
static bool TraceLine_r( int nodeNum, vec3_t origin, vec3_t end, light_trace_t *trace )
{
	traceNode_t		*node;
	int				side;
...
float SetupTrace( light_trace_t *trace )
{
	...
	trace->distance = VectorNormalizeLength2( trace->displacement, trace->direction );
