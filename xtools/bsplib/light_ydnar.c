...
{
	int		i, j, k, f, cs, numVerts, numVotes, fOld;
	...
	shadeAngles = Malloc( numBSPDrawVerts * sizeof( float ) );
	...
	for( i = 0; i < numBSPDrawVerts; i++ )
	{
		...
		for( j = i; j < numBSPDrawVerts && numVerts < MAX_SAMPLES; j++ )
		{
			...
			/* test vertexes */
			if( VectorCompare( yDrawVerts[ i ].xyz, yDrawVerts[ j ].xyz ) == false )
				continue;
			...
			{
				//Msg( "F(%3.3f >= %3.3f) ", RAD2DEG( testAngle ), RAD2DEG( shadeAngle ) );
				continue;
			}
			//Msg( "P(%3.3f < %3.3f) ", RAD2DEG( testAngle ), RAD2DEG( shadeAngle ) );
...
static bool CalcTangentVectors( int numVerts, bspDrawVert_t **dv, vec3_t *stv, vec3_t *ttv )
{
	...
	for( i = 0; i < numVerts; i++ )
	{
		...
		/* debug code */
		//%	MsgDev( D_NOTE, "%d S: (%f %f %f) T: (%f %f %f)\n", i,
...
static void PerturbNormal( bspDrawVert_t *dv, shaderInfo_t *si, vec3_t pNormal, vec3_t stv[ 3 ], vec3_t ttv[ 3 ] )
{
	...
	/* sample normalmap */
	if( RadSampleImage( si->normalImage->pic, dv->st, bump ) == false )
		return;
...
static bool MapTriangle( rawLightmap_t *lm, surfaceInfo_t *info, bspDrawVert_t *dv[ 3 ], bool mapNonAxial )
{
	...
	else if( PlaneFromPoints( plane, dv[ 0 ]->xyz, dv[ 1 ]->xyz, dv[ 2 ]->xyz ) == false )
		return false;
		...
		switch( ds->surfaceType )
		{
			...
			case MST_PATCH:
				...
				/* debug code */
					...
					if( lm->plane )
					{
						Msg( "Planar patch: [%1.3f %1.3f %1.3f] [%1.3f %1.3f %1.3f] [%1.3f %1.3f %1.3f]\n",
	...
	/* debug code */
		...
		for( y = 0; y < lm->sh; y++ )
		{
			for( x = 0; x < lm->sw; x++ )
			{
				...
				/* report bogus origin */
				Msg( "%6d [%2d,%2d] (%4d): XYZ(%+4.1f %+4.1f %+4.1f) LO(%+4.1f %+4.1f %+4.1f) HI(%+4.1f %+4.1f %+4.1f) <%3.0f>\n",
...
void SetupDirt( void )
{
	...
	MsgDev( D_NOTE, "%9d dirtmap vectors\n", numDirtVectors );
}
...
static bool SubmapRawLuxel( rawLightmap_t *lm, int x, int y, float bx, float by, int *sampleCluster, vec3_t sampleOrigin, vec3_t sampleNormal )
{
	...
	else if( (x > 0 && bx <= 0.0f) || (x == (lm->sw - 1) && bx >= 0.0f) )
	{
		...
		Msg( "WARNING: Spurious lightmap S vector\n" );
	...
	else if( (y > 0 && bx <= 0.0f) || (y == (lm->sh - 1) && bx >= 0.0f) )
	{
		...
		Msg( "WARNING: Spurious lightmap T vector\n" );
...
static void SubsampleRawLuxel_r( rawLightmap_t *lm, light_trace_t *trace, vec3_t sampleOrigin, int x, int y, float bias, float *lightLuxel )
{
...
void IlluminateRawLightmap( int rawLightmapNum )
{
	...
	else
	{
		...
		//%		Msg( "Lightmap %9d: 0 lights, axis: %.2f, %.2f, %.2f\n", rawLightmapNum, lm->axis[ 0 ], lm->axis[ 1 ], lm->axis[ 2 ] );
		...
		for( i = 0; i < trace.numLights; i++ )
		{
			...
			if( lm->superLuxels[ lightmapNum ] == NULL )
			{
				/* allocate sampling lightmap storage */
				size = lm->sw * lm->sh * SUPER_LUXEL_SIZE * sizeof( float );
				lm->superLuxels[ lightmapNum ] = Malloc( size );
				memset( lm->superLuxels[ lightmapNum ], 0, size );
			}
			...
			if( lightmapNum > 0 )
			{
				...
				//%	Msg( "Surface %6d has lightstyle %d\n", rawLightmapNum, trace.light->style );
			}
...
void SetupBrushes( void )
{
	...
	if( opaqueBrushes == NULL )
		opaqueBrushes = Malloc( numBSPBrushes / 8 + 1 );
...
void SetupEnvelopes( bool forGrid, bool fastFlag )
{
	...
	while( *owner != NULL )
	{
		...
		else
		{
			...
			if( light->cluster >= 0 )
			{
				...
				/* chop radius against pvs */
				{
					...
					for( i = 0; i < numBSPLeafs; i++ )
					{
						...
						if( leaf->cluster < 0 )
							continue;
						if( ClusterVisible( light->cluster, leaf->cluster ) == false )	/* ydnar: thanks Arnout for exposing my stupid error (this never failed before) */
							continue;
					...
					for( i = 0; i < 3; i++ )
					{
						...
						{
							//% Msg( "WARNING: Light PVS bounds (%.0f, %.0f, %.0f) -> (%.0f, %.0f, %.0f)\ndo not encompass light %d (%f, %f, %f)\n",
							//% 	mins[ 0 ], mins[ 1 ], mins[ 2 ],
							//% 	maxs[ 0 ], maxs[ 1 ], maxs[ 2 ],
					...
					{
						light->envelope = radius;
						//%	MsgDev( D_NOTE, "PVS Cull (%d): culled\n", numLights );
					}
					//%	else
					//%		MsgDev( D_NOTE, "PVS Cull (%d): failed (%8.0f > %8.0f)\n", numLights, radius, light->envelope );
				}
			...
			{
				/* debug code */
				//%	Msg( "Culling light: Cluster: %d Envelope: %f\n", light->cluster, light->envelope );
...
void CreateTraceLightsForBounds( vec3_t mins, vec3_t maxs, vec3_t normal, int numClusters, int *clusters, int flags, light_trace_t *trace )
{
	...
	/* debug code */
	//% Msg( "CTWLFB: (%4.1f %4.1f %4.1f) (%4.1f %4.1f %4.1f)\n", mins[ 0 ], mins[ 1 ], mins[ 2 ], maxs[ 0 ], maxs[ 1 ], maxs[ 2 ] );
	
	/* allocate the light list */
	trace->lights = Malloc( sizeof( light_t* ) * (numLights + 1) );
	trace->numLights = 0;
