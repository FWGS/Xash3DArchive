...
//	RunThreadsOnIndividual (numportals*2, true, BetterPortalVis);
	...
	MsgDev( D_NOTE, "creating leaf vis...\n" );

	for( i = 0; i < portalclusters; i++ )
		ClusterMerge (i);

	MsgDev( D_INFO, "Total visible clusters: %i\n", totalvis );
	MsgDev( D_INFO, "Average clusters visible: %i\n", totalvis / portalclusters );
}
...
void MergeLeaves(void)
{
	...
	MsgDev( D_INFO, "%6d leaves merged\n", totalnummerges );
}
...
void MergeLeafPortals(void)
{
		...
		for (j = 0; j < leaf->numportals; j++)
		{
			...
			for (k = j+1; k < leaf->numportals; k++)
			{
				...
				if (p1->leaf == p2->leaf)
				{
					...
					if (w)
					{
						Mem_Free( p1->winding );	//% FreeWinding(p1->winding);
						p1->winding = w;
						if (p1->hint && p2->hint)
							hintsmerged++;
						p1->hint |= p2->hint;
	...
	MsgDev( D_INFO, "%6d portals merged\n", nummerges );
	MsgDev( D_INFO, "%6d hint portals merged\n", hintsmerged );
}
...
int CountActivePortals( void )
{
	...
	MsgDev( D_INFO, "%6d active portals\n", num );
	MsgDev( D_INFO, "%6d hint portals\n", hints );
...
============
*/
void WriteFloat( file_t *f, float v );
...
//			continue;
		w = p->winding;
		FS_Printf(pf,"%i %i %i ",w->numpoints, 0, 0);
		FS_Printf(pf, "%d ", p->hint);
		for (i=0 ; i<w->numpoints ; i++)
		{
			FS_Printf(pf,"(");
...
void LoadPortals( const char *name )
{
	...
	prtfile = Com_OpenScript( name, NULL, 0 );
	...
	Com_ReadLong( prtfile, true, &numfaces );
	...
	MsgDev( D_INFO, "%4i portalclusters\n", portalclusters );
	MsgDev( D_INFO, "%4i numportals\n", numportals );
	MsgDev( D_INFO, "%6i numfaces\n", numfaces );
	...
	if( numBSPVisBytes > MAX_MAP_VISIBILITY )
		Sys_Break( "MAX_MAP_VISIBILITY limit exceeded\n" );
	...
	for( i = 0, p = portals; i < numportals; i++ )
	{
		...
		if( l->numportals == MAX_PORTALS_ON_LEAF )
			Sys_Break( "Leaf with too many portals\n" );
	...
	for (i = 0, p = faces; i < numfaces; i++)
	{
		...
		if( numpoints > MAX_POINTS_ON_WINDING )
			Sys_Break( "LoadPortals: portal %i has too many points\n", i );
		if(( uint )leafnums[0] > portalclusters )
			Sys_Break( "LoadPortals: reading portal %i\n", i );
...
int VisMain( int argc, char **argv )
{
	...
	for( i = 1; i < (argc - 1); i++ )
	{
		...
		{
			MsgDev( D_INFO, "fastvis = true\n");
			fastvis = true;
		} else if (!strcmp(argv[i], "-merge")) {
			MsgDev( D_INFO, "merge = true\n");
			mergevis = true;
		} else if (!strcmp(argv[i], "-nopassage")) {
			MsgDev( D_INFO, "nopassage = true\n");
			noPassageVis = true;
		} else if (!strcmp(argv[i], "-passageOnly")) {
			MsgDev( D_INFO, "passageOnly = true\n");
			passageVisOnly = true;
		} else if (!strcmp (argv[i],"-nosort")) {
			MsgDev( D_INFO, "nosort = true\n");
			nosort = true;
		} else if (!strcmp (argv[i],"-saveprt")) {
			MsgDev( D_INFO, "saveprt = true\n");
			saveprt = true;
		...
		{
			MsgDev( D_INFO,  "hint = true\n" );
			hint = true;
			mergevis = true;
		}
		else MsgDev( D_WARN, "Unknown option \"%s\"\n", argv[ i ] );
	}

	if( i != argc - 1 )
		Sys_Error( "usage: vis [-threads #] [-level 0-4] [-fast] [-v] bspfile" );
	...
	if( numportals == 0 )
	{
		MsgDev( D_INFO,  "No portals means no vis, exiting.\n" );
		return 0;
	}
	...
	MsgDev( D_INFO,  "visdatasize:%i\n", numBSPVisBytes );
	
	CalcVis();
	
	/* delete the prt file */
	if( !saveprt ) FS_Delete( portalfile );
