...
mapDrawSurface_t *AllocDrawSurface( surfaceType_t type )
{
	...
	if( type <= SURFACE_BAD || type >= NUM_SURFACE_TYPES )
		Sys_Error( "AllocDrawSurface: Invalid surface type %d specified", type );
	
	/* bounds check */
	if( numMapDrawSurfs >= MAX_MAP_DRAW_SURFS )
		Sys_Error( "MAX_MAP_DRAW_SURFS (%d) exceeded", MAX_MAP_DRAW_SURFS );
	ds = &mapDrawSurfs[ numMapDrawSurfs ];
	numMapDrawSurfs++;
	
	/* ydnar: do initial surface setup */
	Mem_Set( ds, 0, sizeof( mapDrawSurface_t ) );
	ds->type = type;
	ds->planeNum = -1;
...
mapDrawSurface_t *CloneSurface( mapDrawSurface_t *src, shaderInfo_t *si )
{
	...
	if( ds->numIndexes <= 0 )
		return ds;
	ds->indexes = Malloc( ds->numIndexes * sizeof( *ds->indexes ) );
...
mapDrawSurface_t *MakeSkyboxSurface( mapDrawSurface_t *src )
{
	...
	{
		Matrix4x4_TransformPoint( skyboxTransform, ds->verts[ i ].xyz );
...
void ClassifySurfaces( int numSurfs, mapDrawSurface_t *ds )
{
	...
	for( ; numSurfs > 0; numSurfs--, ds++ )
	{
		...
		else
		{
			...
			for( i = 0; i < ds->numVerts; i++ )
			{
				...
				if( fabs( dist ) > PLANAR_EPSILON )
				{
					//%	if( ds->planeNum >= 0 )
					//%	{
					//%		Msg( "WARNING: Planar surface marked unplanar (%f > %f)\n", fabs( dist ), PLANAR_EPSILON );
		...
		else
		{
			...
			//%		Msg( "WARNING: Non-planar face (%d): %s\n", ds->planeNum, ds->shaderInfo->shader );
		}
		...
		else
		{
			...
			for( bestAxis = 0; bestAxis < 6; bestAxis++ )
			{
				...
				{
					//% Msg( "Comparing %1.3f %1.3f %1.3f to %1.3f %1.3f %1.3f\n",
			...
			if( bestAxis < 6 )
			{
				...
				//% 	Msg( "Mapped axis %d onto patch\n", bestAxis );
				VectorCopy( axii[ bestAxis ], ds->lightmapAxis );
			}
			
			/* debug code */
			//% if( ds->type == SURFACE_PATCH )
			//% 	Msg( "Failed to map axis %d onto patch\n", bestAxis );
		}
...
shaderInfo_t *GetIndexedShader( shaderInfo_t *parent, indexMap_t *im, int numPoints, byte *shaderIndexes )
{
	...
	if( minShaderIndex == maxShaderIndex )
		...
		com.sprintf( shader, "textures/%s_%dto%d", im->shader, minShaderIndex, maxShaderIndex );
...
mapDrawSurface_t *DrawSurfaceForSide( entity_t *e, brush_t *b, side_t *s, winding_t *w )
{
	...
	if( w->numpoints > MAX_POINTS_ON_WINDING )
		Sys_Error( "DrawSurfaceForSide: w->numpoints = %d (> %d)", w->numpoints, MAX_POINTS_ON_WINDING );
	...
	{
		//%	MsgDev( D_INFO, "Enabling sky hack for shader %s using env %s\n", si->shader, si->skyParmsImageBase );
		com.sprintf( tempShader, "%s_lf", si->skyParmsImageBase );
		DrawSurfaceForShader( tempShader );
		com.sprintf( tempShader, "%s_rt", si->skyParmsImageBase );
		DrawSurfaceForShader( tempShader );
		com.sprintf( tempShader, "%s_ft", si->skyParmsImageBase );
		DrawSurfaceForShader( tempShader );
		com.sprintf( tempShader, "%s_bk", si->skyParmsImageBase );
		DrawSurfaceForShader( tempShader );
		com.sprintf( tempShader, "%s_up", si->skyParmsImageBase );
		DrawSurfaceForShader( tempShader );
		com.sprintf( tempShader, "%s_dn", si->skyParmsImageBase );
		DrawSurfaceForShader( tempShader );
	}
	...
	ds->verts = Malloc( ds->numVerts * sizeof( *ds->verts ) );
	Mem_Set( ds->verts, 0, ds->numVerts * sizeof( *ds->verts ) );
...
static void SubdivideFace_r( entity_t *e, brush_t *brush, side_t *side, winding_t *w, int fogNum, float subdivisions )
{
	...
	if( w == NULL )
		return;
	if( w->numpoints < 3 )
		Sys_Error( "SubdivideFace_r: Bad w->numpoints (%d < 3)", w->numpoints );
...
void SubdivideFaceSurfaces( entity_t *e, tree_t *tree )
{
	...
	for( i = e->firstDrawSurf; i < numBaseDrawSurfs; i++ )
	{
		...
		if( CalcSurfaceTextureRange( ds ) == false )
		{
...
bool SideInBrush( side_t *side, brush_t *b )
{
	...
	if( side->culled == true || (b->compileFlags & C_TRANSLUCENT) )
		return false;
...
static int FilterTrianglesIntoTree( mapDrawSurface_t *ds, tree_t *tree )
{
	...
	for( i = 0; i < ds->numIndexes; i += 3 )
	{
		...
		if( ds->indexes[ i ] >= ds->numVerts ||
			...
			Sys_Error( "Index %d greater than vertex count %d", ds->indexes[ i ], ds->numVerts );
	...
	if( out->firstIndex == numBSPDrawIndexes )
	{
		...
		for( i = 0; i < ds->numIndexes; i++ )
		{
			if( numBSPDrawIndexes == MAX_MAP_DRAW_INDEXES )
				Sys_Error( "MAX_MAP_DRAW_INDEXES" );
			...
			if( ds->type != SURFACE_PATCH )
			{
				...
				{
					Msg( "WARNING: %d %s has invalid index %d (%d)\n",
						numBSPDrawSurfaces,
	...
	if( ds->backSide || ds->shaderInfo->invert )
	{
		...
		for( j = 0; j < ds->patchHeight; j++ )
		{
			for( i = 0; i < (ds->patchWidth / 2); i++ )
			{
				...
				Mem_Copy( &temp, dv1, sizeof( bspDrawVert_t ) );
				Mem_Copy( dv1, dv2, sizeof( bspDrawVert_t ) );
				Mem_Copy( dv2, &temp, sizeof( bspDrawVert_t ) );
			}
		}
...
static void OptimizeTriangleSurface( mapDrawSurface_t *ds )
{
	...
	indexes = Malloc( ds->numIndexes * sizeof( *indexes ) );
...
static void EmitTriangleSurface( mapDrawSurface_t *ds )
{
	...
	else if( (VectorLength( ds->lightmapAxis ) <= 0.0f && ds->planar == false) ||
		ds->type == SURFACE_TRIANGLES ||
		ds->type == SURFACE_FOGHULL ||
		ds->numVerts > maxLMSurfaceVerts ||
		debugSurfaces )
		out->surfaceType = MST_TRISURF;
...
static void MakeDebugPortalSurfs_r( node_t *node, shaderInfo_t *si )
{
	...
	for( c = 0, p = node->portals; p != NULL; c++, p = p->next[ s ] )
	{
		...
		if( w && p->nodes[ 0 ] == node )
		{
			...
			if( PortalPassable( p ) == false )
				continue;
			
			/* check max points */
			if( w->numpoints > 64 )
				Sys_Error( "MakePortalSurfs_r: w->numpoints = %d", w->numpoints );
...
void MakeDebugPortalSurfs( tree_t *tree )
{
	...
	/* note it */
	MsgDev( D_INFO, "--- MakeDebugPortalSurfs ---\n" );
...
void MakeFogHullSurfs( entity_t *e, tree_t *tree, char *shader )
{
	...
	/* note it */
	MsgDev( D_INFO, "--- MakeFogHullSurfs ---\n" );
	...
	ds->numIndexes = 36;
	ds->indexes = Malloc( ds->numIndexes * sizeof( *ds->indexes ) );
	Mem_Set( ds->indexes, 0, ds->numIndexes * sizeof( *ds->indexes ) );
...
int AddSurfaceModelsToTriangle_r( mapDrawSurface_t *ds, surfaceModel_t *model, bspDrawVert_t **tri )
{
	...
	/* subdivide calc */
	{
		...
		{
			float	odds, angle, scale;
			vec3_t	origin, normal, scale3, axis[ 3 ], angles;
			matrix4x4	transform, temp, temp2;
			...
			/* calculate scale */
			scale = model->minScale + Random() * (model->maxScale - model->minScale);
			VectorSet( scale3, scale, scale, scale );
			...
			if( model->oriented )
			{
				...
				if( VectorNormalizeLength2( normal, axis[ 2 ] ) == 0.0f )
				...
				/* copy to matrix */
				Matrix4x4_FromMatrix3x3( temp, axis, scale );

				// rotate around z axis
				Matrix4x4_ConcatEulerAngles( temp, angles, eXYZ );
				
				/* translate */
				Matrix4x4_ConcatTranslate( transform, origin[0], origin[1], origin[2] );
				
				/* tranform into axis space */
				Matrix4x4_Concat( temp2, transform, temp );
				Matrix4x4_Copy( transform, temp2 );
			}
			...
			else
			{
				...
				/* set matrix */
				Matrix4x4_Pivot( transform, origin, angles, eXYZ, scale3, vec3_origin );
			}
...
int AddSurfaceModels( mapDrawSurface_t *ds )
{
	...
	for( model = ds->shaderInfo->surfaceModel; model != NULL; model = model->next )
	{
		...
		switch( ds->type )
		{
			...
			case SURFACE_DECAL:
				/* calculate centroid */
				Mem_Set( &centroid, 0, sizeof( centroid ) );
				alpha = 0.0f;
