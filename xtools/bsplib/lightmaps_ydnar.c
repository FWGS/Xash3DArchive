...
void WriteTGA24( char *filename, byte *data, int width, int height, bool flip )
{
	...
	// DAMN! q3map2 writes 'bad' tga's for some reasons
...
{
	int		i;
	char		dirname[MAX_SYSPATH], filename[MAX_SYSPATH];
	byte		*lightmap;
	...
	for( i = 0, lightmap = bspLightBytes; lightmap < (bspLightBytes + numBSPLightBytes); i++, lightmap += (game->lightmapSize * game->lightmapSize * 3) )
	{
		...
		com.sprintf( filename, "%s/lightmap_%04d.tga", dirname, i );
		Msg( "Writing %s\n", filename );
...
int ExportLightmapsMain( int argc, char **argv )
{
	...
	if( argc < 1 )
	{
		Msg( "Usage: q3map -export [-v] <mapname>\n" );
		return 0;
	}
...
int ImportLightmapsMain( int argc, char **argv )
{
	int 	i, x, y;
	char	dirname[MAX_SYSPATH], filename[MAX_SYSPATH];
	byte	*lightmap, *in, *out;
	rgbdata_t	*pic;
	
	/* arg checking */
	if( argc < 1 )
	{
		Msg( "Usage: q3map -import [-v] <mapname>\n" );
		return 0;
	}
	...
	/* sanity check */
	if( bspLightBytes == NULL ) Sys_Break( "No lightmap data\n" );
	...
	for( i = 0, lightmap = bspLightBytes; lightmap < (bspLightBytes + numBSPLightBytes); i++, lightmap += (game->lightmapSize * game->lightmapSize * 3) )
	{
		...
		com.sprintf( filename, "%s/lightmap_%04d.tga", dirname, i );
		Msg( "Loading %s\n", filename );
		pic = FS_LoadImage( filename, NULL, 0 );

		if( !pic )
		{
			MsgDev( D_WARN, "Unable to load image %s\n", filename );
			continue;
		}
		
		if( pic->width != game->lightmapSize || pic->height != game->lightmapSize )
		{
			MsgDev( D_WARN, "Image %s is not the right size (%d, %d) != (%d, %d)\n", filename, pic->width, pic->height, game->lightmapSize, game->lightmapSize );
			Image_Process( &pic, game->lightmapSize, game->lightmapSize, IMAGE_RESAMPLE );
		}		
		/* copy the pixels */
		in = pic->buffer;
...
static int CompareLightSurface( const void *a, const void *b )
{
	...
	return com.strcmp( asi->shader, bsi->shader );
}
...
void FinishRawLightmap( rawLightmap_t *lm )
{
	...
	lm->lightClusters = Malloc( lm->numLightClusters * sizeof( *lm->lightClusters ) );
	c = 0;
	for( i = 0; i < lm->numLightSurfaces; i++ )
	{
		/* get surface info */
...
bool AddSurfaceToRawLightmap( int num, rawLightmap_t *lm )
{
	...
	if( lm->numLightSurfaces > 1 )
	{
		...
		if( VectorCompare( info->axis, lm->axis ) == false )
			return false;
	...
	{
		MsgDev( D_WARN, "ProjectSurfaceLightmap: Chose a 0 valued axis\n" );
		lm->w = lm->h = 0;
		return false;
	}
	...
	for( n = 0; n < lm->numLightSurfaces; n++ )
	{
		...
		for( i = 0; i < ds2->numVerts; i++ )
		{
			...
			{
				MsgDev( D_NOTE, "WARNING: Lightmap texture coords out of range: S %1.4f > %3d || T %1.4f > %3d\n",
					s, lm->w, t, lm->h );
			}
		}
	}
	...
	if( VectorLength( ds->lightmapVecs[ 2 ] ) || 1 )	/* ydnar: can't remember what exactly i was thinking here... */
	{
		...
		lm->vecs = Malloc( 3 * sizeof( vec3_t ) );
		...
		for( i = 0; i < 2; i++ )
		{
			len = VectorNormalizeLength2( vecs[ i ], normalized );
...
void SetupSurfaceLightmaps( void )
{
	...
	sortSurfaces = Malloc( numBSPDrawSurfaces * sizeof( int ) );
	...
	for( i = 0; i < numBSPModels; i++ )
	{
		...
		for( j = 0; j < model->numBSPSurfaces; j++ )
		{
				...
				for( s = 0; s < leaf->numBSPLeafSurfaces; s++ )
				{
					if( bspLeafSurfaces[ leaf->firstBSPLeafSurface + s ] == num )
					{
						if( numSurfaceClusters >= maxSurfaceClusters )
							Sys_Break( "maxSurfaceClusters exceeded\n" );
			...
			{
				/* make a plane */
				info->plane = Malloc( 4 * sizeof( float ) );
			...
			if( ds->surfaceType == MST_TRISURF || ds->surfaceType == MST_FOLIAGE || (info->si->compileFlags & C_VERTEXLIT) )
				numSurfsVertexLit++;
			else
			{
				numSurfsLightmapped++;
	...
	numLightSurfaces = 0;
	lightSurfaces = Malloc( numSurfsLightmapped * sizeof( int ) );
	memset( lightSurfaces, 0, numSurfsLightmapped * sizeof( int ) );
	
	/* allocate a list of raw lightmaps */
	numRawSuperLuxels = 0;
	numRawLightmaps = 0;
	rawLightmaps = Malloc( numSurfsLightmapped * sizeof( *rawLightmaps ) );
	...
	for( i = 0; i < numBSPDrawSurfaces; i++ )
	{
		...
		while( added )
		{
			...
			added = false;
			for( j = i + 1; j < numBSPDrawSurfaces && lm->finished == false; j++ )
			{
				/* get info and attempt early out */
				num2 = sortSurfaces[ j ];
				ds2 = &bspDrawSurfaces[ num2 ];
	...
	{
		vertexLuxels[ k ] = Malloc( numBSPDrawVerts * VERTEX_LUXEL_SIZE * sizeof( float ) ); 
		memset( vertexLuxels[ k ], 0, numBSPDrawVerts * VERTEX_LUXEL_SIZE * sizeof( float ) );
		radVertexLuxels[ k ] = Malloc( numBSPDrawVerts * VERTEX_LUXEL_SIZE * sizeof( float ) );
		memset( radVertexLuxels[ k ], 0, numBSPDrawVerts * VERTEX_LUXEL_SIZE * sizeof( float ) );
	}
	
	/* emit some stats */
	MsgDev( D_NOTE, "%9d surfaces\n", numBSPDrawSurfaces );
	MsgDev( D_NOTE, "%9d raw lightmaps\n", numRawLightmaps );
	MsgDev( D_NOTE, "%9d surfaces vertex lit\n", numSurfsVertexLit );
	MsgDev( D_NOTE, "%9d surfaces lightmapped\n", numSurfsLightmapped );
	MsgDev( D_NOTE, "%9d planar surfaces lightmapped\n", numPlanarsLightmapped );
	MsgDev( D_NOTE, "%9d non-planar surfaces lightmapped\n", numNonPlanarsLightmapped );
	MsgDev( D_NOTE, "%9d patches lightmapped\n", numPatchesLightmapped );
	MsgDev( D_NOTE, "%9d planar patches lightmapped\n", numPlanarPatchesLightmapped );
}
...
{
	int		i, j, x, y, x2, y2, *cluster, *cluster2, numStitched, numCandidates, numLuxels, f, fOld;
	double		start;
	...
	MsgDev( D_NOTE, "%9d luxels stitched\n", numStitched );
}
...
static void SetupOutLightmap( rawLightmap_t *lm, outLightmap_t *olm )
{
	...
	/* allocate buffers */
	olm->lightBits = Malloc( (olm->customWidth * olm->customHeight / 8) + 8 );
	memset( olm->lightBits, 0, (olm->customWidth * olm->customHeight / 8) + 8 );
	olm->bspLightBytes = Malloc( olm->customWidth * olm->customHeight * 3 );
	memset( olm->bspLightBytes, 0, olm->customWidth * olm->customHeight * 3 );
	if( deluxemap )
	{
		olm->bspDirBytes = Malloc( olm->customWidth * olm->customHeight * 3 );
		...
		for( y = 0; y < yMax; y++ )
		{
			for( x = 0; x < xMax; x++ )
			{
				...
				if( deluxemap )
				{
					...
					if( VectorNormalizeLength2( deluxel, direction ))
					{
						/* encode [-1,1] in [0,255] */
...
void StoreSurfaceLightmaps( void )
{
	...
	FS_StripExtension( dirname );
	Mem_Set( rgbGenValues, 0, sizeof( rgbGenValues ));
	Mem_Set( alphaGenValues, 0, sizeof( alphaGenValues ));
	...
	for( i = 0; i < numRawLightmaps; i++ )
	{
		...
		for( lightmapNum = 0; lightmapNum < MAX_LIGHTMAPS; lightmapNum++ )
		{
			...
			{
				size = lm->w * lm->h * BSP_LUXEL_SIZE * sizeof( float );
				lm->bspLuxels[ lightmapNum ] = Malloc( size );
			...
			if( bounce )
			{
				...
				if( lm->radLuxels[ lightmapNum ] == NULL )
					lm->radLuxels[ lightmapNum ] = Malloc( size );
	...
	if( sortLightmaps == NULL )
		sortLightmaps = Malloc( numRawLightmaps * sizeof( int ) );
	...
	if( outLightmaps != NULL )
	{
		for( i = 0; i < numOutLightmaps; i++ )
		{
			Mem_Free( outLightmaps[ i ].lightBits );
			Mem_Free( outLightmaps[ i ].bspLightBytes );
		}
		Mem_Free( outLightmaps );
		outLightmaps = NULL;
	}
	...
	for( i = 0; i < numOutLightmaps; i++ )
	{
		...
		if( olm->lightmapNum < 0 || olm->extLightmapNum >= 0 || externalLightmaps )
		{
			...
			/* write lightmap */
			com.sprintf( filename, "%s/" EXTERNAL_LIGHTMAP, dirname, numExtLightmaps );
			MsgDev( D_NOTE, "\nwriting %s", filename );
	...
	for( i = numExtLightmaps; i; i++ )
	{
		...
		com.sprintf( filename, "%s/" EXTERNAL_LIGHTMAP, dirname, i );
		...
		if( olm != NULL && lm != NULL && lm->styles[ 1 ] != LS_NONE && game->load != LoadRBSPFile )	//%	info->si->styleMarker > 0 )
		{
			...
			for( lightmapNum = 1; lightmapNum < MAX_LIGHTMAPS; lightmapNum++ )
			{
				...
				if( lm->outLightmapNums[ lightmapNum ] == lm->outLightmapNums[ 0 ] )
					...
					com.sprintf( lightmapName, "maps/%s/" EXTERNAL_LIGHTMAP, mapName, olm->extLightmapNum );
				
				/* get rgbgen string */
				if( rgbGenValues[ style ] == NULL )
				{
					com.sprintf( key, "_style%drgbgen", style );
				...
				rgbGen[ 0 ] = '\0';
				if( rgbGenValues[ style ][ 0 ] != '\0' )
					com.sprintf( rgbGen, "\t\trgbGen %s // style %d\n", rgbGenValues[ style ], style );
				else
					rgbGen[ 0 ] = '\0';
			...
			/* store it */
			//%	Msg( "Emitting: %s (%d", csi->shader, strlen( csi->shader ) );
