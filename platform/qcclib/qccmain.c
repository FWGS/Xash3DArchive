...
void PR_BspModels (void)
{
	...
	p = FS_CheckParm ("-bspmodels");
	if (!p)
		return;
	if (p == fs_argc-1)
		Sys_Error("-bspmodels must preceed a game directory");
	gamedir = fs_argv[p+1];
...
int WriteSourceFiles(int h, dprograms_t *progs, bool sourceaswell)
{
...
void PR_WriteData (int crc)
{
	...
	for (def = pr.def_head.next ; def ; def = def->next)
	{
		...
		{
			if (def->constant)
				PR_Warning(WARN_NOTREFERENCEDCONST, strings + def->s_file, def->s_line, "%s  no references", def->name);
			else
				PR_Warning(WARN_NOTREFERENCED, strings + def->s_file, def->s_line, "%s  no references", def->name);
...
//			continue;
	...
	Msg("Writing %s\n", destfile);
	h = VFS_OpenWrite (destfile, 2*1024*1024);
	VFS_Write (h, &progs, sizeof(progs));
	VFS_Write (h, "\r\n\r\n", 4);
	VFS_Write (h, v_copyright, strlen(v_copyright)+1);
	VFS_Write (h, "\r\n\r\n", 4);
	while(VFS_Seek (h, 0, SEEK_CUR) & 3)//this is a lame way to do it
	{
		VFS_Write (h, "\0", 1);
	}

	progs.ofs_strings = VFS_Seek (h, 0, SEEK_CUR);
	...
	progs.ofs_statements = VFS_Seek (h, 0, SEEK_CUR);
...
#define statements16 ((dstatement16_t*) statements)
	...
	progs.ofs_functions = VFS_Seek (h, 0, SEEK_CUR);
	...
	{
	case 32:
		progs.ofs_globaldefs = VFS_Seek (h, 0, SEEK_CUR);
		...
		progs.ofs_fielddefs = VFS_Seek (h, 0, SEEK_CUR);
...
#define fields16 ((ddef16_t*)fields)
	...
	progs.ofs_globals = VFS_Seek (h, 0, SEEK_CUR);
	...
	case QCF_FTEDEBUG:
		...
		progs.ofsbodylessfuncs = VFS_Seek (h, 0, SEEK_CUR);
		progs.numbodylessfuncs = WriteBodylessFuncs(h);		

		if (debugtarget)
		{
			progs.ofslinenums = VFS_Seek (h, 0, SEEK_CUR);
		...
		if (types)
		{
			progs.ofs_types = VFS_Seek (h, 0, SEEK_CUR);
...
def_t	*PR_DefForFieldOfs (gofs_t ofs)
{
	...
	for (d=pr.def_head.next ; d ; d=d->next)
	{
		if (d->type->type != ev_field)
			continue;
		if (*((unsigned int *)&pr_globals[d->ofs]) == ofs)
			return d;
	}
...
//	pr_warning[WARN_ASSIGNMENTTOCONSTANT] = true;
	...
	if (FS_CheckParm("-nowarn") || FS_CheckParm("-Wnone"))
		memset(pr_warning, 1, sizeof(pr_warning));
	if (FS_CheckParm("-Wall"))
		memset(pr_warning, 0, sizeof(pr_warning));

	if (FS_CheckParm("-h2"))
		pr_warning[WARN_CASEINSENSATIVEFRAMEMACRO] = true;
...
int PR_FindQCFiles()
{
	...
	do
	{
		filelist[numfiles] = Qalloc (strlen(fd.cFileName)+1);
		strcpy(filelist[numfiles], fd.cFileName);
		numfiles++;
	} while(FindNextFile(h, &fd)!=0);
	FindClose(h);
#else
	Msg("-Facc is not supported on this platform. Please make a progs.src file instead\n");
#endif
...
void PR_main ( void )//as part of the quake engine
{
	...
	p = FS_FileSize("qcc.cfg");
	if (p < 0) p = FS_FileSize("src/qcc.cfg");
	if (p > 0)
	{
		s = Qalloc(p+1);
		s = FS_LoadFile("qcc.cfg", &p);

		while(1)
		{
			s = SC_ParseToken(s);
			if (!strcmp(token, "MAX_REGS"))
			{
				s = SC_ParseToken(s);
				MAX_REGS = atoi(token);
			} else if (!strcmp(token, "MAX_STRINGS")) {
				s = SC_ParseToken(s);
				MAX_STRINGS = atoi(token);
			} else if (!strcmp(token, "MAX_GLOBALS")) {
				s = SC_ParseToken(s);
				MAX_GLOBALS = atoi(token);
			} else if (!strcmp(token, "MAX_FIELDS")) {
				s = SC_ParseToken(s);
				MAX_FIELDS = atoi(token);
			} else if (!strcmp(token, "MAX_STATEMENTS")) {
				s = SC_ParseToken(s);
				MAX_STATEMENTS = atoi(token);
			} else if (!strcmp(token, "MAX_FUNCTIONS")) {
				s = SC_ParseToken(s);
				MAX_FUNCTIONS = atoi(token);
			} else if (!strcmp(token, "MAX_TYPES")) {
				s = SC_ParseToken(s);
				maxtypeinfos = atoi(token);			
			} else if (!strcmp(token, "MAX_TEMPS")) {
				s = SC_ParseToken(s);
				max_temps = atoi(token);			
			} else if (!strcmp(token, "CONSTANTS")) {
				s = SC_ParseToken(s);
				MAX_CONSTANTS = atoi(token);
			}
			else if (!s)
				break;
			else
				Msg("Bad token in qcc.cfg file\n");
		}
	}
	...
	else if (p < 0)
	{
		...
		FS_WriteFile("qcc.cfg", s, strlen(s));
	}
	*/
...
//	optres_comexprremoval = 0;
	...
	if(FS_CheckParm("-reacc"))
	{
		flag_acc = 1;
          	flag_caseinsensative = 1;
	}
...
memset(pr_immediate_string, 0, sizeof(pr_immediate_string));
	...
	if ( FS_CheckParm ("/?") || FS_CheckParm ("?") || FS_CheckParm ("-?") || FS_CheckParm ("-help") || FS_CheckParm ("--help"))
	{
		Msg ("qcc looks for progs.src in the current directory.\n");
		Msg ("to look in a different directory: qcc -src <directory>\n");
//		Msg ("to build a clean data tree: qcc -copy <srcdir> <destdir>\n");
//		Msg ("to build a clean pak file: qcc -pak <srcdir> <packfile>\n");
//		Msg ("to bsp all bmodels: qcc -bspmodels <gamedir>\n");
		Msg ("-Fwasm <funcname> causes FTEQCC to dump all asm to qc.asm\n");
		Msg ("-O0 to disable optimisations\n");
		Msg ("-O1 to optimise for size\n");
		Msg ("-O2 to optimise more - some behaviours may change\n");
		Msg ("-O3 to optimise lots - experimental or non-future-proof\n");
		Msg ("-Oname to enable an optimisation\n");
		Msg ("-Ono-name to disable optimisations\n");
		Msg ("-Kkeyword to activate keyword\n");
		Msg ("-Kno-keyword to disable keyword\n");
		Msg ("-Wall to give a stupid number of warnings\n");
		Msg ("-Ttarget to set a output format\n");
		Msg ("-Fautoproto to enable automatic prototyping\n");
	...
	{
		Msg("Compiling without case sensativity\n");
	...
	if (p && p < fs_argc-1 )
	{
		strcpy (qccmsourcedir, fs_argv[p+1]);
	...
	PR_BeginCompilation ((void *)Qalloc (0x100000), 0x100000);

	if (flag_acc)
	{
		if (!PR_FindQCFiles())
			Sys_Error ("Couldn't open file for asm output.");
	}
	else
	{
		p = FS_CheckParm ("-qc");
		if (!p || p >= fs_argc-1 || fs_argv[p+1][0] == '-')
			p = FS_CheckParm ("-srcfile");
		if (p && p < fs_argc-1 )
			sprintf (qccmprogsdat, "%s%s", qccmsourcedir, fs_argv[p+1]);		
		else
		{	//look for a preprogs.src... :o)
			sprintf (qccmprogsdat, "%spreprogs.src", qccmsourcedir);
			if (FS_FileSize(qccmprogsdat) <= 0)
	...
	if (writeasm)
	{
		asmfile = FS_Open("qc.asm", "wb", true, false );
	...
	if (FS_CheckParm ("-qc"))
	{
		...
		FS_StripExtension(destfile);
		strcat(destfile, ".qco");

		p = FS_CheckParm ("-o");
		if (!p || p >= fs_argc-1 || fs_argv[p+1][0] == '-')
		if (p && p < fs_argc-1 )
			sprintf (destfile, "%s%s", qccmsourcedir, fs_argv[p+1]);
		goto newstyle;
	}
...
void new_PR_ContinueCompile(void);
...
void PR_ContinueCompile(void)
{
	...
	if (newstylesource)
	{
		new_PR_ContinueCompile();
		return;
	}

	qccmsrc = SC_ParseToken(qccmsrc);
	if (!qccmsrc)
	{
		if (autoprototype)
		{
...
extern string_t	s_file, s_file2;
...
void StartNewStyleCompile(void)
{
	...
	pr_file_p = qccmsrc;
	s_file = s_file2 = PR_CopyString (compilingfile);
