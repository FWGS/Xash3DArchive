	...
	//				eval = false;
		...
		else if (!strncmp(directive, "pragma", 6))
		{
			...
			else if (!stricmp(token, "keyword") || !stricmp(token, "flag"))
			{
				...
				int st;
				s = SC_ParseToken(msg);
				if (!stricmp(token, "enable") || !stricmp(token, "on"))
					st = 1;
				else if (!stricmp(token, "disable") || !stricmp(token, "off"))
					st = 0;
				else
				{
						...
						if (!stricmp(compiler_flag[f].abbrev, token))
						{
							if (compiler_flag[f].flags & FLAG_MIDCOMPILE)
								*compiler_flag[f].enabled = st;
								...
								PR_ParseWarning(WARN_BADPRAGMA, "Cannot enable/disable keyword/flag via a pragma");
							break;
						}
					}
			...
			else if (!stricmp(token, "warning"))
			{
				...
				if (st>=0)
				{
					...
					{
						if (st == 2)	//toggle
							pr_warning[wn] = true - pr_warning[wn];
						else
							pr_warning[wn] = st;
					}
				}
				...
				PR_ParseWarning(WARN_BADPRAGMA, "Unknown pragma \'%s\'", token);
		}
		return true;
	}
				...
				*buffer = '\0';
				...
				for(;;)
				{
					...
					if (*pr_file_p == '#')	//if you ask for #a##b you will be shot. use #a #b instead, or chain macros.
					{
						...
						{	//stringify
							pr_file_p++;
							pr_file_p = SC_ParseWord(pr_file_p);
							if (!pr_file_p)
								break;
				...
				PR_ParseError(ERR_TOOFEWPARAMS, "Macro without opening brace");
		}
		else
			PR_IncludeChunk(c->value, false, NULL);
	...
	if (!PR_CheckToken (")"))
	{
		if (PR_CheckToken ("..."))
			...
			do
			{
				...
				if (PR_CheckToken("arg"))
				{
					sprintf(argname, "arg%i", ftype->num_parms);
					name = argname;
					nptype = PR_NewType("Variant", ev_variant);
				}
				else if (PR_CheckToken("vect"))	//this can only be of vector sizes, so...
				{
					sprintf(argname, "arg%i", ftype->num_parms);
					name = argname;
					nptype = PR_NewType("Vector", ev_vector);
				}
				else
				{
					name = PR_ParseName();
