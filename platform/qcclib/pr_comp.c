...
bool flag_acc;		//reacc like behaviour of src files (finds *.qc in start dir and compiles all in alphabetical order)
bool flag_caseinsensative;	//symbols will be matched to an insensative case if the specified case doesn't exist. This should b usable for any mod
...
// if the token is an immediate, allocate a constant for it
	...
	if (PR_CheckToken("["))	//reacc support
...
void PR_ParseStatement (void)
{
	...
	if (PR_CheckKeyword(keyword_exit, "exit"))
	{
		PR_FreeTemp(PR_Statement (&pr_opcodes[OP_DONE], 0, 0, NULL));
		PR_Expect (";");
		return;
...
// hexenC has void name() : 2;
	...
	if (PR_CheckKeyword(keyword_external, "external"))
	{	//reacc style builtin
		if (pr_token_type != tt_immediate
		|| pr_immediate_type != type_float
		|| pr_immediate._float != (int)pr_immediate._float)
...
//
	...
	{
		if (PR_CheckKeyword (keyword_var, "var"))	//reacc support
		{	//parse lots of locals
			char *name;
			do {
				name = PR_ParseName();
				PR_Expect(":");
				e2 = PR_GetDef(PR_ParseType(false), name, pr_scope, true, 1);
				PR_Expect(";");
			} while(!PR_CheckToken("{"));
		}
		else
...
void PR_ParseDefs (char *classname)
{
	...
	{
		char *oldp;
		if (PR_CheckKeyword (keyword_codesys, "CodeSys"))	//reacc support.
		{
			extern int ForcedCRC;
			if (ForcedCRC)
				PR_ParseError(ERR_BADEXTENSION, "progs crc was already specified - only one is allowed");
		...
		if (PR_CheckKeyword (keyword_var, "var"))	//reacc support.
		{
			...
			{
				if (!PR_GetDef(type_void, "end_sys_fields", NULL, false, 0))
					PR_GetDef(type_void, "end_sys_fields", NULL, true, 1);
			}
		...
		if (PR_CheckKeyword (keyword_function, "function"))	//reacc support.
		{
			accglobalsblock = 2;
		}
		if (PR_CheckKeyword (keyword_objdata, "objdata"))	//reacc support.
		{
			if (accglobalsblock == 3)
			{
				if (!PR_GetDef(type_void, "end_sys_fields", NULL, false, 0))
				...
				if (!PR_GetDef(type_void, "end_sys_globals", NULL, false, 0))
					PR_GetDef(type_void, "end_sys_globals", NULL, true, 1);
			accglobalsblock = 3;
		}
	}
	...
	case 1:
		...
		char *oldp = pr_file_p;
		name = PR_ParseName();
		if (!PR_CheckToken(":"))	//nope, it wasn't!
		{
			PR_IncludeChunk(name, true, NULL);
			PR_Lex();
			PR_UnInclude();
			pr_file_p = oldp;
			break;
		}
		if (PR_CheckKeyword(keyword_object, "object"))
			PR_GetDef(type_entity, name, NULL, true, 1);
		else if (PR_CheckKeyword(keyword_string, "string"))
			PR_GetDef(type_string, name, NULL, true, 1);
		else if (PR_CheckKeyword(keyword_real, "real"))
		{
			def = PR_GetDef(type_float, name, NULL, true, 1);
			if (PR_CheckToken("="))
			{
		...
		else if (PR_CheckKeyword(keyword_vector, "vector"))
		{
			def = PR_GetDef(type_vector, name, NULL, true, 1);
			if (PR_CheckToken("="))
			{
				PR_Expect("[");
		...
		else if (PR_CheckKeyword(keyword_pfunc, "pfunc"))
			PR_GetDef(type_function, name, NULL, true, 1);
		else
			PR_ParseError(ERR_BADNOTTYPE, "Bad type\n");
		PR_Expect (";");

		if (PR_CheckKeyword (keyword_system, "system"))
			PR_Expect (";");
		return;
		}
	case 2:
	...
	case 3:
		...
		if (PR_CheckKeyword(keyword_object, "object"))
			PR_GetDef(PR_FieldType(type_entity), name, NULL, true, 1);
		else if (PR_CheckKeyword(keyword_string, "string"))
			PR_GetDef(PR_FieldType(type_string), name, NULL, true, 1);
		else if (PR_CheckKeyword(keyword_real, "real"))
			PR_GetDef(PR_FieldType(type_float), name, NULL, true, 1);
		else if (PR_CheckKeyword(keyword_vector, "vector"))
			PR_GetDef(PR_FieldType(type_vector), name, NULL, true, 1);
		else if (PR_CheckKeyword(keyword_pfunc, "pfunc"))
			PR_GetDef(PR_FieldType(type_function), name, NULL, true, 1);
		else
			PR_ParseError(ERR_BADNOTTYPE, "Bad type\n");
		PR_Expect (";");
		return;
	...
	if (!pr_scope && PR_CheckKeyword(keyword_function, "function"))	//reacc support.
	{
		name = PR_ParseName ();
		PR_Expect("(");
		type = PR_ParseFunctionTypeReacc(false, type);
		PR_Expect(";");
		if (!stricmp(name, "null"))
			Msg("null!\n");
		def = PR_GetDef (type, name, NULL, true, 1);
		...
		{	//ignore the code and stuff
			...
			else
			{
				...
				while (!PR_CheckToken("{"))	//skip over the locals.
				{
					if (pr_token_type == tt_eof)
					{
						PR_ParseError(0, "Unexpected EOF");
						break;
					}
					PR_Lex();
				}
...
//		PR_ParseError ("Fields must be global");
