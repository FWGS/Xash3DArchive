...
=================
*/
game_export_t DLLEXPORT *ServerAPI (game_import_t *import)
{
	gi = *import;
...
void EndDMLevel (void)
{
	...
	if (*sv_maplist->string) {
		...
		t = strtok(s, seps);
		while (t != NULL) {
			if (strcasecmp(t, level.mapname) == 0) {
...
void CheckNeedPass (void)
{
	...
	if (password->modified || spectator_password->modified) 
	{
		...
		if (*password->string && strcasecmp(password->string, "none"))
			need |= 1;
		if (*spectator_password->string && strcasecmp(spectator_password->string, "none"))
			need |= 2;
