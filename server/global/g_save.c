...
field_t	fields[] = 
{
	...
	{"skyname", STOFS(sky), F_LSTRING, FFL_SPAWNTEMP},
...
void InitGame (void)
{
	...
	zone_game = Mem_AllocPool("Zone Game");
	...
	sprintf (game.helpmessage1, "");

	sprintf (game.helpmessage2, "");
...
void WriteGame (vfile_t *f, bool autosave)
{
	...
	if(developer->value) gi.dprintf ("==== WriteGame ====\n");

	if (!autosave) game.transition_ents = 0;
...
void ReadGame (vfile_t *f)
{
	...
	if(developer->value) gi.dprintf ("==== ReadGame ====\n");
...
void WriteLevel (vfile_t *f)
{
	...
	if(developer->value) gi.dprintf ("==== WriteLevel ====\n");
