...
void SV_CalcBlend (edict_t *ent)
{
	...
	{
		ent->client->ps.rdflags &= ~RDF_UNDERWATER;
		ent->client->ps.rdflags &= ~RDF_WATER;
		ent->client->ps.rdflags &= ~RDF_SLIME;
		ent->client->ps.rdflags &= ~RDF_LAVA;
          }
	...
	if (contents & (/*CONTENTS_SOLID|*/CONTENTS_LAVA))
	{
		...
		else ent->client->ps.rdflags |= RDF_LAVA;
	}
	else if (contents & CONTENTS_SLIME)
	{
		if(!r_motionblur->value)
			SV_AddBlend (0.0, 0.1, 0.05, 0.6, ent->client->ps.blend);
		else ent->client->ps.rdflags |= RDF_SLIME;
	}
	else if (contents & CONTENTS_WATER)
	{
		if(!r_motionblur->value)
			SV_AddBlend (0.5, 0.3, 0.2, 0.4, ent->client->ps.blend);
		else ent->client->ps.rdflags |= RDF_WATER;
	}
	...
	//					ent->client->damage_alpha, ent->client->ps.blend); //shit!
...
//	ent->s.renderfx = 0;
	...
	if (ent->client->flashlight)
	{
		...
		if(level.flashlight_cost > 0) {
			if(!strcasecmp(FLASHLIGHT_ITEM,"health") || 
