...
bool is_backing_up (edict_t *train)
{
			...
			if(!next)
			{
				...
				for(i=maxclients->value; i<globals.num_edicts && !next; i++)
				{
					...
					if(e->class_id != ENTITY_PATH_TRACK)
						continue;
					if(e->target && !strcasecmp(e->target,path->targetname))
					{
						next = e;
						VectorSubtract(next->s.origin,path->s.origin,to_next);
						VectorNormalize(to_next);
...
//							path->prevpath = next;
					...
					if(!next && e->target2 && !strcasecmp(e->target2,path->targetname))
					{
						next = e;
						VectorSubtract(next->s.origin,path->s.origin,to_next);
						VectorNormalize(to_next);
	...
	else	// Moving forward
	{
		...
		{	// Check for path_tracks that target (or target2) this path_track.
			...
			for(i=maxclients->value; i<globals.num_edicts && !next; i++)
			{
				...
				if(e->target2 && !strcasecmp(e->target2,path->targetname))
				{
					edict_t	*next2;
					float	dot2;
...
void SP_func_tracktrain (edict_t *self)
{
	...
	if(self->sounds > 0)
	{
		if(self->sounds > 9)
			self->sounds = 9;
		self->source = TagMalloc(10,TAG_LEVEL);
...
void find_tracktrain (edict_t *self)
{
	...
	if(train->sounds > 0)
	{
		train->source = TagMalloc(10,TAG_LEVEL);
