...
void WriteTransitionEdict (file_t *f, edict_t *changelevel, edict_t *ent)
{
	byte		*temp;
	edict_t		e;
	field_t		*field;
	...
	if(e.classname &&
	   ( !strcasecmp(e.classname,"misc_actor") || strstr(e.classname,"monster_") ) &&
...
int trigger_transition_ents (edict_t *changelevel, edict_t *self)
{
	...
	for(i=game.maxclients+1; i<globals.num_edicts; i++)
	{
		...
		if( (ent->class_id == ENTITY_FUNC_TRACKTRAIN) && !(ent->spawnflags & 8) && ent->targetname)
		{
			...
			e = G_Spawn();
			e->classname = TagMalloc(17,TAG_LEVEL);
			strcpy(e->classname,"info_train_start");
			e->targetname = TagMalloc(strlen(ent->targetname)+1,TAG_LEVEL);
			strcpy(e->targetname,ent->targetname);
			e->target = TagMalloc(strlen(ent->target)+1,TAG_LEVEL);
	...
	for(i=game.maxclients+1; i<globals.num_edicts; i++)
	{
		...
		for(p=DoNotMove, nogo=false; p->name && !nogo; p++)
			if(!strcasecmp(ent->classname,p->name))
				nogo = true;
		if(nogo) continue;
