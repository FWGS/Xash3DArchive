...
void SP_target_speaker (edict_t *ent)
{
	if(!(ent->spawnflags & 8))
	{
		...
		{
			ent->message = TagMalloc(strlen(st.noise)+5,TAG_LEVEL);
			sprintf(ent->message,"%s.wav", st.noise);
		}
		else
		{
			ent->message = TagMalloc(strlen(st.noise)+1,TAG_LEVEL);
...
void use_target_changelevel (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	if(self->spawnflags & 2 && activator->client)
	{
		...
		while(transition)
		{
			if(!strcasecmp(transition->targetname,self->targetname))
			{
...
void use_target_splash (edict_t *self, edict_t *other, edict_t *activator)
{
	MESSAGE_BEGIN (svc_temp_entity);
		...
		WRITE_BYTE (self->count);
		WRITE_COORD (self->s.origin);
		WRITE_DIR (self->movedir);
		WRITE_BYTE (self->sounds);
	MESSAGE_SEND (MSG_PVS, self->s.origin, NULL);
	if (self->dmg)
...
void target_effect_steam (edict_t *self, edict_t *activator)
{
	...
	MESSAGE_BEGIN (svc_temp_entity);
		...
		WRITE_COORD (self->s.origin);
		WRITE_DIR (self->movedir);
		WRITE_BYTE (self->sounds&0xff);
		WRITE_SHORT ( (int)(self->speed) );
		WRITE_LONG ( (int)(wait) );
	MESSAGE_SEND (MSG_PVS, self->s.origin, NULL);
...
void target_effect_trail (edict_t *self, edict_t *activator)
{
	...
	MESSAGE_BEGIN(svc_temp_entity);
		WRITE_BYTE(self->style);
		if((self->style == TE_PARASITE_ATTACK) || (self->style==TE_MEDIC_CABLE_ATTACK) || (self->style == TE_HEATBEAM) || (self->style==TE_MONSTER_HEATBEAM) || (self->style == TE_GRAPPLE_CABLE))
		{
			WRITE_SHORT(self-g_edicts);
		}
		WRITE_COORD(self->s.origin);
		...
		{
			WRITE_COORD(vec3_origin);
		}
	MESSAGE_SEND(MSG_PVS, self->s.origin, NULL);

	if(level.num_reflectors)
	{
		if((self->style == TE_RAILTRAIL) || (self->style == TE_BUBBLETRAIL) || (self->style == TE_BFG_LASER) || (self->style == TE_DEBUGTRAIL) || (self->style == TE_BUBBLETRAIL2))
...
void target_effect_lightning(edict_t *self, edict_t *activator)
{
	...
	MESSAGE_BEGIN (svc_temp_entity);
		WRITE_BYTE (self->style);
		WRITE_SHORT (target - g_edicts);		// destination entity
		WRITE_SHORT (self - g_edicts);		// source entity
		WRITE_COORD (target->s.origin);
		WRITE_COORD (self->s.origin);
	MESSAGE_SEND (MSG_PVS, self->s.origin, NULL);
}
...
void target_effect_tunnel_sparks (edict_t *self, edict_t *activator)
{
	...
	for (i = 0; i < self->count; i++)
	{
		...
		MESSAGE_BEGIN (svc_temp_entity);
			...
			WRITE_BYTE (1);
			WRITE_COORD (origin);
			WRITE_DIR (vec3_origin);
			WRITE_BYTE (self->sounds + (rand()&7));  // color
		MESSAGE_SEND (MSG_PVS, self->s.origin, NULL);
	}
}
//===============================================================================
...
void target_animation_use (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	if(self->spawnflags & 1)
	{
		if(activator && activator->client)
			return;
		if(self->message && strcasecmp(self->message, activator->classname))
			return;
		if(!self->target)
			target = activator;
	}
...
void SP_target_animation (edict_t *self)
{
	...
	move = TagMalloc(sizeof(mmove_t), TAG_LEVEL);
...
/*=============================================================================
 TARGET_SKY - uses the "skyname" string as parameter to "skyname" console command.
...
void SP_target_sky (edict_t *self)
{
	...
	self->pathtarget = TagMalloc(strlen(st.sky)+1,TAG_LEVEL);
...
void clone (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	if(!parent)
		return;
	child = G_Spawn();
	child->classname = TagMalloc(strlen(parent->classname)+1,TAG_LEVEL);
