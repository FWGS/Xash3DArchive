...
bool M_SetDeath(edict_t *self, mmove_t **deathmoves)
{
	mmove_t	*move=NULL;
	mmove_t *dmove;
...
void monster_use (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	if (activator->client)
	{
		...
		if(self->dmgteam && !strcasecmp(self->dmgteam,"player"))
			self->dmgteam = NULL;
	}
...
bool monster_start (edict_t *self)
{
	...
	if (self->spawnflags & SF_MONSTER_GOODGUY) {
		self->monsterinfo.aiflags |= AI_GOOD_GUY;
		if(!self->dmgteam) {
			self->dmgteam = TagMalloc(8*sizeof(char), TAG_LEVEL);
