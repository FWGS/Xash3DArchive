...
bool actor_checkattack (edict_t *self)
{
	vec3_t		v;
...
bool actor_blocked (edict_t *self, float dist)
{
...
void SP_misc_actor (edict_t *self)
{
	...
	if( (!strcasecmp(self->usermodel,"male")) ||
		(!strcasecmp(self->usermodel,"female")) ||
		(!strcasecmp(self->usermodel,"cyborg"))    )
	{
	...
	{
		if(!strcasecmp(self->usermodel,ActorNames[i]))
			ActorID = i+1;
	}
...
void target_actor_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (self->spawnflags & 1)		//jump
	{
		...
		// the jump.
//		if (!strcasecmp(other->classname,"misc_actor"))
	...
	for(i=game.maxclients+1; i<globals.num_edicts; i++) {
		...
		if(num_actors > 0) {
			for(j=0; j<num_actors && (s_match == 0 || w_match[0] == 0 || w_match[1] == 0); j++) {
				...
				if(!strcasecmp(e->usermodel,e0->usermodel)) {
					s_match = j+1;
		...
		if(!s_match) {
			...
			if(!strcasecmp(e->usermodel,"male") || !strcasecmp(e->usermodel,"female")) {
