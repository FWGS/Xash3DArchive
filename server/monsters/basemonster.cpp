...
//IMPLEMENT_SAVERESTORE( CBaseMonster, CBaseAnimating );
...
{
	SetNextThink( 0.05 );// keep monster thinking.
...
// perhaps MaintainActivity() or a ShiftAnimationOverTime() or something.
	...
	if ( !MovementIsComplete())Move( flInterval );
}
		...
		case MOVEGOAL_PATHCORNER:
			{
				...
				while ( pPathCorner && i < ROUTE_SIZE )
				{
					...
					pPathCorner = pPathCorner->GetNext();
...
void CBaseMonster :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case SCRIPT_EVENT_FIREEVENT:		// Fire a trigger
		UTIL_FireTargets( pEvent->options, this, this, USE_TOGGLE );
		break;
...
*/
	...
	if ( fFireTarget )
	{
		...
		UTIL_FireTargets( m_iszTriggerTarget , this, this, USE_TOGGLE );
...
int CBaseMonster :: TakeArmor (float flArmor, int suit)
{
	if (!pev->takedamage) return 0;
	return CBaseEntity::TakeArmor(flArmor, suit);
}
...
BOOL CBaseMonster :: HasHumanGibs( void )
{
	...
	if ( myClass == CLASS_NONE || myClass == CLASS_MACHINE || myClass == CLASS_PLAYER_BIOWEAPON && myClass == CLASS_ALIEN_BIOWEAPON)
	{
		return FALSE;
	}
	else
...
void CBaseMonster :: GibMonster( void )
{
	...
	{
		CGib::SpawnHeadGib( this, iszCustomGibs );
		CGib::SpawnRandomGibs( this, 4, 1, iszCustomGibs );
		CGib::SpawnStickyGibs (this, 4, iszCustomGibs );
		gibbed = TRUE;
	}
	...
	if ( !IsPlayer() && gibbed ) UTIL_Remove (this);
	else
	{
		StopAnimation();
		pev->velocity = g_vecZero;
...
{
	if ( ( iGib == GIB_NORMAL && pev->health < -30 ) || ( iGib == GIB_ALWAYS ) )
		return TRUE;
...
void CMonsterTarget :: Spawn ( void )
{
	if (pev->spawnflags & SF_MONSTERTARGET_OFF)
		pev->health = 0;
	else	pev->health = 1; // Don't ignore me, I'm not dead. I'm quite well really. I think I'll go for a walk...
	SetBits (pev->flags, FL_MONSTER);
}
...
void CMonsterTarget::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	{
		if(pev->health) useType = USE_OFF;
		else useType = USE_ON;
	}
	if (useType == USE_ON)pev->health = 1;
          else if(useType == USE_OFF)pev->health = 0;
}