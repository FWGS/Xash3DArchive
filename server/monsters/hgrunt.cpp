...
BOOL CHGrunt :: CheckRangeAttack2 ( float flDot, float flDist )
{
	...
	else
	{
		...
		// estimate position
		if (HasConditions( bits_COND_SEE_ENEMY))
			vecTarget = vecTarget + ((vecTarget - pev->origin).Length() / HGRUNT_GRENADE_SPEED) * m_hEnemy->pev->velocity;
	}
	...
	{
		Vector vecToss = VecCheckThrow( pev, GetGunPosition(), vecTarget, HGRUNT_GRENADE_SPEED, 0.5 );
...
void CHGrunt :: Shoot ( void )
{
	...
	if (m_cAmmoLoaded > 0)
	{
		...
		FireBullets(1, vecShootOrigin, vecShootDir, VECTOR_CONE_10DEGREES, 2048, BULLET_MP5 ); // shoot +-5 degrees
...
void CHGrunt :: Shotgun ( void )
{
	...
	FireBullets(HGRUNT_SHOTGUN_PELLETS, vecShootOrigin, vecShootDir, VECTOR_CONE_15DEGREES, 2048, BULLET_BUCKSHOT, 0 ); // shoot +-7.5 degrees
	...
	switch( pEvent->event )
	{
		...
		case HGRUNT_AE_GREN_TOSS:
		{
			...
			if (m_pCine)
			{
				...
				if (vecToss == g_vecZero)
				{
					vecToss = (gpGlobals->v_forward*0.5+gpGlobals->v_up*0.5).Normalize()*HGRUNT_GRENADE_SPEED;
				}
		...
		case HGRUNT_AE_GREN_LAUNCH:
		{
			...
			if (m_pCine)
			{
				Vector vecToss;
				if (m_hTargetEnt != NULL && m_pCine->PreciseAttack())
					vecToss = VecCheckThrow( pev, GetGunPosition(), m_hTargetEnt->pev->origin, HGRUNT_GRENADE_SPEED, 0.5 );
				else
				{
			...
			m_fThrowGrenade = FALSE;
			m_flNextGrenadeCheck = gpGlobals->time + RANDOM_FLOAT( 4, 6 );// wait six seconds before even looking again to see if a grenade can be thrown.
		}
		break;
		...
		case HGRUNT_AE_KICK:
		{
			...
			if ( pHurt )
			{
				...
				pHurt->TakeDamage( pev, pev, HGRUNT_DMG_KICK, DMG_CLUB );
			}
		}
		break;
