...
//	ALERT( at_console, "Sequence %s finished\n", STRING(pev->targetname));//STRING( m_pCine->m_iszPlay ) );
	...
	UTIL_FireTargets( pev->target, NULL, this, USE_TOGGLE );
}
...
//				ALERT(at_console, "Undelaying start\n");
				...
				if (pTarget->m_iDelay <= 0)
				{
					...
					UTIL_FireTargets( m_iszFireOnBegin, this, this, USE_TOGGLE ); //LRC
...
class CScriptedSentence : public CBaseAnimating
{
public:
	void Spawn( void );
...
IMPLEMENT_SAVERESTORE( CScriptedSentence, CBaseAnimating );
