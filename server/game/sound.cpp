...
void TEXTURETYPE_Init()
{
	...
	pMemFile = g_engfuncs.pfnLoadFile( "sound/materials.txt", &fileSize );
	if ( !pMemFile )
		return;
...
// hit the world, try to play sound based on texture material type
	...
	CBaseEntity *pEntity = CBaseEntity::Instance(ptr->pHit);
          CBaseBrush *pBrush = pEntity->MyBrushPointer();
          
	if(pBrush && pBrush->m_Material) return MatVolume(pBrush->m_Material);
	...
	case CHAR_TEX_FLESH:
		...
		if (iBulletType == BULLET_CROWBAR) return 0.0;
	...
	if (chTextureType == CHAR_TEX_COMPUTER)
	{
		...
		if ( ptr->flFraction != 1.0 && RANDOM_LONG(0,1))
		{
			...
			{
				case 0: UTIL_EmitAmbientSound(ENT(0), ptr->vecEndPos, "materials/spark5.wav", flVolume, ATTN_NORM, 0, 100); break;
				case 1: UTIL_EmitAmbientSound(ENT(0), ptr->vecEndPos, "materials/spark6.wav", flVolume, ATTN_NORM, 0, 100); break;
			}
		}
	}
...
class CFMODAudio : public CBaseLogic
{
public:
	void Spawn( void );
...
LINK_ENTITY_TO_CLASS( ambient_music, CFMODAudio );

void CFMODAudio::Spawn( void )
{
	...
	if(pev->spawnflags & SF_LOOP) SetBits( pev->impulse, SF_LOOP );
}
