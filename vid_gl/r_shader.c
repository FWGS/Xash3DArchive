...
void Shader_TouchImages( ref_shader_t *shader, e_free free_unused )
{
	...
	for( i = 0; i < shader->num_stages; i++ )
	{
		...
		for( j = 0; j < stage->num_textures; j++ )
		{
			...
			{
				// because even 'force' mode can't unload system textures
				R_FreeImage( texture );
			}
			else
			{				
...
//		pass->glState = GLSTATE_SRCBLEND_ONE|GLSTATE_DSTBLEND_ZERO;
	...
	case SHADER_TEXTURE:
		...
		else
		{
			...
			{
				pass->tcMods = (tcMod_t *)((byte *)shader->name + length + 1 + sizeof( ref_stage_t ) * shader->num_stages );
				pass->tcMods[0].type = TCMOD_CONVEYOR;
				pass->numtcMods++;
			}
