...
{
	if( uiAdvControls.invertMouse.enabled && Cvar_VariableValue( "m_pitch" ) > 0 )
		Cvar_SetValue( "m_pitch", -Cvar_VariableValue( "m_pitch" ));
	else if( !uiAdvControls.invertMouse.enabled && Cvar_VariableValue( "m_pitch" ) < 0 )
		Cvar_SetValue( "m_pitch", fabs( Cvar_VariableValue( "m_pitch" )));

	Cvar_SetValue( "crosshair", uiAdvControls.crosshair.enabled );
	Cvar_SetValue( "freelook", uiAdvControls.mouseLook.enabled );
	Cvar_SetValue( "lookspring", uiAdvControls.lookSpring.enabled );
	Cvar_SetValue( "lookstrafe", uiAdvControls.lookStrafe.enabled );
	Cvar_SetValue( "m_filter", uiAdvControls.mouseFilter.enabled );
	Cvar_SetValue( "sv_aim", uiAdvControls.autoaim.enabled );
	Cvar_SetValue( "m_sensitivity", (uiAdvControls.sensitivity.curValue * 20.0f) + 0.1f );
...
static void UI_AdvControls_GetConfig( void )
{
	...
	if( Cvar_VariableInteger( "crosshair" ))
		uiAdvControls.crosshair.enabled = 1;

	if( Cvar_VariableInteger( "freelook" ))
		uiAdvControls.mouseLook.enabled = 1;

	if( Cvar_VariableInteger( "lookspring" ))
		uiAdvControls.lookSpring.enabled = 1;

	if( Cvar_VariableInteger( "lookstrafe" ))
		uiAdvControls.lookStrafe.enabled = 1;

	if( Cvar_VariableInteger( "m_filter" ))
		uiAdvControls.mouseFilter.enabled = 1;

	if( Cvar_VariableInteger( "sv_aim" ))
		uiAdvControls.autoaim.enabled = 1;

	uiAdvControls.sensitivity.curValue = (Cvar_VariableValue( "m_sensitivity" ) - 0.1f) / 20.0f;
...
{
	Mem_Set( &uiAdvControls, 0, sizeof( uiAdvControls_t ));
