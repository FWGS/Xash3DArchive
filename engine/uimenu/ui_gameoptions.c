...
static void UI_GameOptions_UpdateConfig( void )
{
	...
	com.snprintf( fpsText, sizeof( fpsText ), "%.f", uiGameOptions.maxFPS.curValue );
...
=================
*/
static const char *UI_GameOptions_KeyFunc( int key, bool down )
{
...
static void UI_GameOptions_GetConfig( void )
{
	...
	if( Cvar_VariableInteger( "allow_download" ))
...
{
	Mem_Set( &uiGameInitial, 0, sizeof( uiGameValues_t ));
