...
void PRVM_MEM_Alloc(void)
{
	...
	prog->edictprivate_size = max(prog->edictprivate_size, (int)sizeof(edict_state_t));
	...
	{
		prog->edicts[i].priv.ed = (edict_state_t *)((byte *)prog->edictprivate + i * prog->edictprivate_size);
		prog->edicts[i].fields.vp = (void*)((byte *)prog->edictsfields + i * prog->edict_size);
	}
}
...
void VM_Error (const char *fmt, ...)
{
	...
	Com_Error (ERR_DROP, "Prvm error: %s", msg);
}
...
{
	return pi->Mem.Alloc(prog->progs_mempool, buffersize, filename, fileline);
}

void _PRVM_Free(void *buffer, const char *filename, int fileline)
{
	pi->Mem.Free(buffer, filename, fileline);
}
...
void _PRVM_FreeAll(const char *filename, int fileline)
{
	...
	prog->fielddefs = NULL;
	prog->functions = NULL;
	pi->Mem.EmptyPool(prog->progs_mempool, filename, fileline);
}
