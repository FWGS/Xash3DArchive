...
{
	AngleVectorsRight(PRVM_G_VECTOR(OFS_PARM0), prog->globals.server->v_forward, prog->globals.server->v_right, prog->globals.server->v_up);
}

void PF_makevectors2 (void)
{
	AngleVectorsLeft(PRVM_G_VECTOR(OFS_PARM0), prog->globals.server->v_forward, prog->globals.server->v_right, prog->globals.server->v_up);
}
...
void PF_setorigin (void)
{
	...
	VectorCopy (org, e->fields.sv->origin);
	SV_LinkEdict (e);
}


void SetMinMaxSize (prvm_edict_t *e, float *min, float *max, bool rotate)
{
	int		i;
	...
	// set derived values
	VectorCopy (min, e->fields.sv->mins);
	VectorCopy (max, e->fields.sv->maxs);
	VectorSubtract (max, min, e->fields.sv->size);
...
void PF_traceline (void)
{
	...
	prog->globals.server->trace_inopen = (trace.contents & MASK_SHOT) ? true : false; // this is correct?
...
int PF_newcheckclient (int check)
{
	...
	for (;; i++)
	{
		...
		if (i != check && (ent->priv.sv->free || ent->fields.sv->health <= 0 || ((int)ent->fields.sv->flags & FL_NOTARGET)))
			continue;
	...
	VectorAdd(ent->fields.sv->origin, ent->fields.sv->view_ofs, org);
...
void PF_checkclient (void)
{
	...
	if (ent->priv.sv->free || ent->fields.sv->health <= 0)
	{
	...
	VectorAdd(self->fields.sv->origin, self->fields.sv->view_ofs, view);
...
void PF_findradius (void)
{
	...
	for (i = 0; i < numtouchedicts; i++)
	{
		...
		if (ent->fields.sv->solid == SOLID_NOT)	continue;
		VectorSubtract(org, ent->fields.sv->origin, eorg);
		VectorMAMAM(1, eorg, 0.5f, ent->fields.sv->mins, 0.5f, ent->fields.sv->maxs, eorg);
		if (DotProduct(eorg, eorg) < radius2)
		{
			ent->fields.sv->chain = PRVM_EDICT_TO_PROG(chain);
			chain = ent;
		}
	}
...
void PF_walkmove (void)
{
	...
	if (!((int)ent->fields.sv->flags & (FL_ONGROUND|FL_FLY|FL_SWIM)))
		return;
...
void PF_droptofloor (void)
{
	...
	VectorCopy (ent->fields.sv->origin, end);
	end[2] -= 256;

	trace = SV_Trace(ent->fields.sv->origin, ent->fields.sv->mins, ent->fields.sv->maxs, end, ent, MASK_ALL );
...
void PF_aim (void)
{
	...
	VectorCopy (ent->fields.sv->origin, start);
	start[2] += 20;
	...
	if (tr.ent && ((prvm_edict_t *)tr.ent)->fields.sv->takedamage == DAMAGE_AIM && (flags & DF_NO_FRIENDLY_FIRE || ent->fields.sv->team <=0 || ent->fields.sv->team != ((prvm_edict_t *)tr.ent)->fields.sv->team))
	{
		VectorCopy (prog->globals.server->v_forward, PRVM_G_VECTOR(OFS_RETURN));
		return;
	}
	...
	{
		prog->xfunction->builtinsprofile++;
		if (check->fields.sv->takedamage != DAMAGE_AIM)
			continue;
		if (check == ent)
			continue;
		if (flags & DF_NO_FRIENDLY_FIRE && ent->fields.sv->team > 0 && ent->fields.sv->team == check->fields.sv->team)
			continue;	// don't aim at teammate
		for (j = 0; j < 3; j++)
			end[j] = check->fields.sv->origin[j] + 0.5 * (check->fields.sv->mins[j] + check->fields.sv->maxs[j]);
	...
	if (bestent)
	{
		VectorSubtract (bestent->fields.sv->origin, ent->fields.sv->origin, dir);
		dist = DotProduct (dir, prog->globals.server->v_forward);
...
void PF_changeyaw (void)
{
	...
	current = anglemod(ent->fields.sv->angles[1]);
	ideal = ent->fields.sv->ideal_yaw;
	speed = ent->fields.sv->yaw_speed;
	...
	ent->fields.sv->angles[1] = anglemod(current + move);
}
...
void PF_changepitch (void)
{
	...
	current = anglemod( ent->fields.sv->angles[0] );
	...
	ent->fields.sv->angles[0] = anglemod(current + move);
}
...
void PF_WriteDir (void)
{
	MSG_WriteDir (&sv.multicast, PRVM_G_VECTOR(OFS_PARM0));
}
...
void PF_setspawnparms (void)
{
	...
	for (i = 0; i < NUM_SPAWN_PARMS; i++)
		prog->globals.server->parm[i] = client->spawn_parms[i];
}
...
void PF_copyentity (void)
{
	...
	Mem_Copy(out->fields.sv, in->fields.sv, prog->progs->entityfields * 4);
}

static cmodel_t *getmodel(prvm_edict_t *ed)
{
	int	modelindex;
	if (!ed || ed->priv.sv->free)
		return NULL;

	modelindex = (int)ed->fields.sv->modelindex;
...
PF_WriteDir,				// #62 void(vector v) WriteDir 
...
PF_tracetoss,				// #79 void(entity e, entity ignore) tracetoss (DP_QC_TRACETOSS)
