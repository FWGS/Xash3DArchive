...
bool ConvJPG(const char *name, char *buffer, int filesize )
{
	...
	pic.width = jpg_file.width;
	pic.height = jpg_file.height;
	pic.type = PF_RGBA_32;
	if(!Lump_ValidSize((char *)name, &pic, 2048, 2048 ))
		return false;

	pic.size = jpg_file.width * jpg_file.height * 4;
	jpg_file.data = Mem_Alloc( zonepool, pic.size );
