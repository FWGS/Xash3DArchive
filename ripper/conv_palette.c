...
void Conv_SetPalette( byte *pal, uint *d_table )
{
	int	i;
	byte	rgba[4];
...
void Conv_GetPaletteD1( void )
{
	...
	if(!d1palette_init)
	{
		Conv_SetPalette( palette_d1, d_8toD1table );
...
void Conv_GetPaletteQ1( void )
{
	...
	if(!q1palette_init)
	{
		Conv_SetPalette( palette_q1, d_8toQ1table );
...
void Conv_GetPaletteQ2( void )
{
	...
	if(!q2palette_init)
	{
		Conv_SetPalette( palette_q2, d_8toQ2table );
...
void Conv_GetPaletteLMP( byte *pal, int rendermode )
{
	...
	else if(rendermode == LUMP_QFONT)
	{
		...
		Conv_SetPalette( palette_q1, d_8to24table );
		d_8to24table[0] = 0;
		d_currentpal = d_8to24table;
	}
	else Conv_GetPaletteQ1(); // default quake palette          
}
...
bool ConvPAL( const char *name, char *buffer, int filesize )
{
	...
	FS_WriteFile( va("%s.pal", name ), d_currentpal, 1024 ); 
	return true;
}
...
NOTE: must call Conv_GetPaletteXXX before used
============
*/
bool Conv_Copy8bitRGBA(const byte *in, byte *out, int pixels)
{
	...
	if(!d_currentpal)
	{
		MsgDev(D_ERROR,"Conv_Copy8bitRGBA: no palette set\n");
		return false;
	}
	if(!in)
	{
		MsgDev(D_ERROR,"Conv_Copy8bitRGBA: no input image\n");
		return false;
	}
