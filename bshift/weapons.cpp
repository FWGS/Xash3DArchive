...
void CBasePlayerWeapon::ItemPostFrame( void )
{
	...
	if ((m_pPlayer->pev->button & IN_ATTACK2) && CanAttack( m_flNextSecondaryAttack, gpGlobals->time, FALSE ) )
	{
	...
	else if ((m_pPlayer->pev->button & IN_ATTACK) && CanAttack( m_flNextPrimaryAttack, gpGlobals->time, FALSE ) )
	{
...
int CBasePlayerWeapon::UpdateClientData( CBasePlayer *pPlayer )
{
	...
	if ( m_iClip != m_iClientClip || 
		 state != m_iClientWeaponState || 
		  Q_rint( m_iClientFov ) != Q_rint( m_pPlayer->pev->fov ))
	{
		bSend = TRUE;
	}
